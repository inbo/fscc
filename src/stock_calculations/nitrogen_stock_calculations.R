...
# Calculate nitrogen stocks until 100 cm (or until the effective soil depth
# if shallower)
# ---------------------------------------------------------------------------
...
if (calculate_new_stocks == TRUE) {
  ...
  for (survey_form_i in c("s1", "so")) {
    ...
    data <- read.csv(paste0(dir,
                            survey_form_i,
                            "_profile_n_stocks.csv"),
                     sep = ";")

    assign_env(paste0(survey_form_i, "_profile_n_stocks"),
               data)
    ...
    assign_env(paste0(survey_form_i, "_plot_n_stocks"),
               data)
...
plot_stocks <- get_env(paste0("so", "_plot_",
...
graph_interval(data = plot_n_stocks %>%
...
map_icpf2(layers = "data_so",
          title = paste0("**Forest soil nitrogen stock**",
                         " · ",
                         "Level II<br>",
          ...
          legend_title = paste0("**Nitrogen stock**<br>",
                                "t N ha<sup>-1</sup>"),
          biogeo_palette = NULL,
          legend_classes = NULL,
...
map_icpf2(layers = "data_so",
          ...
          legend_title = paste0("**Annual increase**<br>",
                                "nitrogen stock<br>",
                                "t N ha<sup>-1</sup> year<sup>-1</sup>"),
          biogeo_palette = NULL,
          legend_classes = NULL,
...
### 8.3.3. Nitrogen Distribution Index (NDI) ----
