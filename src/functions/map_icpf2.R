...
map_icpf2 <- function(layers,
  ...
  # visualisation of continuous variables (stocks and stock changes).
  # I placed it in a separate script for the time being - it is still chaotic
  # and needs to be integrated with the map_icpf function script.
  ...
  if (grepl("change", variable_continuous)) {

    scale_function <- scale_color_scico(
      palette = "vik",
      direction = -1,
  ...
  if (!is.null(legend_classes)) {
    ...
    base_map <- base_map +
      # Map the points of the first sf layer
      geom_sf(data = spat_layer_1,
              aes(color = .data[[variable_continuous]]),
              size = point_size) +
      scale_function
  ...
  if (length(layers) == 1) {
    base_map <- base_map +
      coord_sf(xlim = c(2400000, 5800000), #6000000
               ylim = c(ylim_min, 5400000)) #1000000
