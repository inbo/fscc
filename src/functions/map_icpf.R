...
map_icpf <- function(layers,
  ...
  biogeo_sf <- 
    read_sf("./data/additional_data/shapefiles/BiogeoRegions2016_shapefile/BiogeoRegions2016.shp")
  ...
  base_map <-
    ...
    theme(
      ...
      plot.margin = margin(t = 0.5,  # Set the top margin of the plot to 0.5 cm
                           r = 0.5,  # Set the right margin of the plot to 0.5 cm
                           b = 0.5,  # Set the bottom margin of the plot to 0.5 cm
                           l = 0.5,  # Set the left margin of the plot to 0.5 cm
                           # Set the unit of measurement for margins to centimeters
                           unit = "cm")) + 
    # Map the borders of the countries (without fill)
    geom_sf(data = world_spat, color = "black", fill = NA, linewidth = 0.5) +
    ...
    geom_sf(data = spat_layer_1, 
            aes(color = legend_classes[1]), # Colour should refer to the legend class
    ...
    ggspatial::annotation_scale(
      ...
      line_width = 1, 
      # Set the vertical distance between the scale bar and the edge of the panel
      pad_y = unit(0.3, "cm")) +
    ...
    ggspatial::annotation_north_arrow(
      ...
      width = unit(1, "cm"), 
      # Set the vertical distance between the north arrow and the edge of the panel
      pad_y = unit(1, "cm")) +
    ...
    # coord_sf(xlim = c(2400000,6000000), ylim = c(1000000,5400000)) +
  ...
  azores_map <- 
    ...
    theme(legend.title = element_text(size = 10), 
          ...
          # Set the background color of the plot panel using the sea_col variable
          panel.background = element_rect(fill = sea_col)) +
    geom_sf(data = world_spat, color = "black", fill = NA, linewidth = 0.5) +
    geom_sf(data = spat_layer_1, 
  ...
  if (length(layers) == 1) {
    ...
    # coord_sf(xlim = c(1005419,7846148), ylim = c(562857,5455103))
  ...
  # Add the second + third layer and coordinate system in case of three layers ----
  ...
  full_map <-
    ...
    # Draw base map
    draw_plot(base_map) + 
    # Draw the left rectangle on which the Canary Islands inset map will be plotted
    draw_grob(
