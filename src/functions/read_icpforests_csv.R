...
#' @details
#' This function imports survey forms of the specified survey, along with some additional
#' dataframes containing ICP Forests soil data. Although the function is customised
#' for the soil surveys of ICP Forests, it can also be used to import the other surveys.
#' The function expects the survey forms to be downloaded from the ICP Forests 
#' website and organised in a specific folder structure.
#'
#' The function also creates a dataframe with the most common coordinates for each plot in the survey,
#' assuming these are the "correct" coordinates. It is recommended to first import data for the system
#' installment surveys before importing data from other surveys, as the coordinates of non-system
#' installment surveys depend on the coordinates of system installment surveys in the global environment.
#' 
#' Requirements:
#' 
...
#'      original MM or SS is not in the 0-59 range, or an NA if this is not the case
#'    
...
#'        - "unique_survey_profile": "partner_code" + "survey_year" + "code_plot" +
#'          "profile_pit_id" ("pfh" survey forms)
#'        - "unique_survey_layer": "partner_code" + "survey_year" + "code_plot" +
#'          "code_layer" ("som" survey forms); or: "partner_code" + "survey_year" +
...
assertthat::assert_that(code_survey %in% names(list_data_tables),
                        msg = "The given survey code does not exist in ICP Forests")

# Identify the level of the survey (Level I versus Level II)

if (code_survey %in% c("s1", "y1", "bd", "c1", "f1")) {survey_level <- "LI"};
if (code_survey %in% c("so", "si", "sw", "ss", "lf",
                       "aq", "cc", "dp", "fo", "gr",
                       "gv", "gb", "la", "li", "mm",
                       "oz", "ph")) {survey_level <- "LII"}
...
assertthat::assert_that(class(download_date) == "character" &&
                        ...
                        msg = "The given download_date should be a character string with format 'YYYYMMDD'")
...
# Import inconsistency_catalogue and attribute_catalogue_pir and save to the global environment
...
{data_availability <- 
 ...
 data_availability$plot_ID <- 
   paste0(data_availability[, which(names(data_availability) == "partner_code")],
          "_",
 ...
 for (j in 1:nrow(data_availability))
   
 {data_availability$country[j] <- 
   d_country$lib_country[match(data_availability$code_country[j], d_country$code)];
  data_availability$partner_short[j] <- 
   d_partner$desc_short[match(data_availability$partner_code[j], d_partner$code)];
  data_availability$partner[j] <- 
   d_partner$description[match(data_availability$partner_code[j], d_partner$code)]};
...
{ # Read the data table
  ...
  vec_as_factor <- which(names(df) %in% 
                         c("code_layer", "code_texture_class", "origin", "horizon_master",
                           "horizon_subordinate", "colour_moist", "colour_dry",
                           "code_horizon_texture_class", "profile_pit_ID", "profile_pit_id",
                           "code_wrb_soil_group", "code_wrb_qualifier_1",
                           ...
                           "diagnostic_1", "diagnostic_2", "diagnostic_3", "diagnostic_4",
                           "diagnostic_5", "diagnostic_6", "diagnostic_7", "diagnostic_8",
                           "diagnostic_9", "diagnostic_10", "code_wrb_publication",
                           "code_parameter", "code_pretreatment", "code_sample_prep",
                           "code_determination", "laboratory_id", "sw_id",
                           "code_depth_layer", "pfh_id", "code_event_type",
                           "product_material"));
  
  for (j in vec_as_factor) {df[,j] <- as.factor(df[,j])};
  ...
  if (!identical(vec_as_date, integer(0))) {
    for (j in vec_as_date) {df[,j] <- as.character(as.Date(df[,j]))}}
  
  # Add a column with download_date
  df$download_date <- as.character(as.Date(parsedate::parse_iso_8601(download_date)))
  
  # For s1_som and so_som: add column "layer type"
  if ((code_survey == "so" | code_survey == "s1") & 
     list_data_tables[[which(names(list_data_tables) == code_survey)]][i] == "som")
    
    {layer_type_levels <- list(peat = c("H", "H05", "H12", "H24", "H51", "Hxx", "H1",
                                        "H2", "HFH", "HH", "HL", "H01", "H48", "HFS",
                                        "HS", "HF"), 
                          forest_floor = c("O", "O1", "O2", "O3", "OF", "OFH", "OH",
                                           "OL", "OLF"), 
  ...
  if ((code_survey == "so" | code_survey == "s1") & 
      list_data_tables[[which(names(list_data_tables) == code_survey)]][i] == "pfh")
    
     {df$layer_type <- NA;
      for (j in 1:nrow(df))
     {if (!is.na(df$horizon_master[j])) {
        if (str_detect(df$horizon_master[j], "^O")) # if horizon_master starts with O
     {df$layer_type[j] <- "forest_floor"} else
       
      if (str_detect(df$horizon_master[j], "^H")) # if horizon_master starts with H
      {df$layer_type[j] <- "peat"} else
  ...
  if ((code_survey == "sw") & 
      list_data_tables[[which(names(list_data_tables) == code_survey)]][i] == "swc")
    
  {df$layer_type <- NA;
  for (j in 1:nrow(df))
  {if (str_detect(df$code_depth_layer[j], "^O")) # if horizon_master starts with O
  {df$layer_type[j] <- "forest_floor"} else
    
    if (str_detect(df$code_depth_layer[j], "^H")) # if horizon_master starts with H
    {df$layer_type[j] <- "peat"} else
    {df$layer_type[j] <- "mineral"}};
  ...
  if (!identical(which(names(vec) == "partner_code"), integer(0)) &
      ...
      !identical(which(names(vec) == "code_plot"), integer(0)))
  {df$unique_survey <- paste0(df[,vec[which(names(vec) == "partner_code")]], "_",
  ...
  {df$unique_layer_repetition <- 
                       paste0(df[,vec[which(names(vec) == "partner_code")]], "_",
                              df[,vec[which(names(vec) == "survey_year")]], "_",
                              df[,vec[which(names(vec) == "code_plot")]], "_",
                              df[,vec[which(names(vec) == "code_layer")]], "_",
                              df[,vec[which(names(vec) == "repetition")]])};
...
assign(paste0(code_survey, "_",
              list_data_tables[[which(names(list_data_tables) == code_survey)]][i]), 
       df, envir = globalenv())}
...
# Create a dataframe with the most common coordinates for each plot of the survey
...
if (survey_level == "LII" &&
  ...
  surveys <- c(surveys,
               paste0("si", "_",
                      list_data_tables[[which(names(list_data_tables) == "si")]]))}
...
coordinates_full <- get(survey_forms_with_coordinates[1], envir = .GlobalEnv) %>%
...
if (length(survey_forms_with_coordinates) > 1) {
  for (i in 2:length(survey_forms_with_coordinates)) {
    coordinates_full_i <- get(survey_forms_with_coordinates[i], envir = .GlobalEnv) %>%
      select(plot_ID, longitude_dec, latitude_dec)
