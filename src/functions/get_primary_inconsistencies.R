...
#' (lower case and separated by '_', e.g. 'so_som') or survey (lower case, e.g. 'so') 
#' to be evaluated
#' @param solve Logical - Indicates whether the function should rename "code_layer"
#' in 's1_som' in case of ambiguous (non-unique) code_layers (if TRUE).
#' 
#' @details 
...
#'   > Copy the original column "code_layer" to a new column "code_layer_original"
...
get_primary_inconsistencies <- function(code_survey, solve = NULL) {
  ...
  # - "code_survey" (the name of the survey data to process):
  #   "code_survey" can be the code of a survey (e.g. "s1"),
  #   but can also be the name of a survey form (e.g. "s1_som")!
  # - "solve": the argument "solve" tells the function to rename "code_layer" 
  #   in "s1_som" in case of ambiguous (non-unique) code_layers (solve == TRUE).
  ...
  # Outputs - This function generates and returns:
  # - an inconsistency report ("list_primary_inconsistencies")
  # - in case of ambiguous (non-unique) code_layers (i.e. possibly for "s1_som")
  #   (FSCC_47): the original data form, but with renamed "code_layer"
  #   (if solve == TRUE)
  ...
  #    > Update unique identifiers that include "code_layer"
  #   Note: this is only for internal use (to avoid conflicts in primary key),
  #   and needs to be harmonised in the manual
  ...
  # The intention is to create a list_primary_inconsistencies of the following format
  # to store inconsistencies found in the survey data:

    list_primary_inconsistencies <- data.frame(survey_form = NULL, 
                                           partner = NULL, 
                                           partner_code = NULL, 
                                           country = NULL,
                                           code_country = NULL, 
  ...
  for (i in 1:length(list_data_tables[[which(names(list_data_tables) == 
                                             code_survey)]])) {
    ...
    # FSCC_47: Check for "som" data forms whether code_layer is unique within a given profile
    # (based on any_code_layer_ambiguities() function)
    ...
    if (solve == TRUE) {df$code_layer_original <- as.character(df$code_layer);
    df$code_layer <- as.character(df$code_layer)}
    
    if ("code_layer_original" %in% names(df)) {
      col_code_layer <- df$code_layer_original} else {
    ...
    for (j in seq_along(layers_duplicated)) {
      ...
      # If there are different values in layer_limit_superior or layer_limit_inferior
      # Or if they are all NA
      ...
      if ((length(unique(df$layer_limit_superior[j_dupl])) == length(j_dupl)) ||
        ...
        if (solve == TRUE) {
          ...
          initial_letters <- gsub("[0-9x]+$", "", 
                                  unique(as.character(df$code_layer_original[j_dupl])))
          ...
          df$code_layer[j_dupl] <-
            paste0(initial_letters,
              ...
              abs(df$layer_limit_inferior[j_dupl]))} else {
        ...
        # Store information about the inconsistency in "list_primary_inconsistencies"
        ...
        list_primary_inconsistencies <- rbind(list_primary_inconsistencies, 
                                              data.frame(survey_form = (rep(survey_form,length(j_dupl))), 
                                                         ...
                                                         rule_ID = rule_ID,
                                                         non_duplicated_error_type_per_record = non_duplicated_error_type_inconsistency,
                                                         change_date = df$change_date[j_dupl],
                                                         download_date = rep(download_date_pir,length(j_dupl))));
    ...
    if ("code_layer_original" %in% names(df)) {
      column_code_layer_horizon_master <- df[, which(names(df) == "code_layer_original")]} else
    if ("code_layer" %in% names(df)) {
      column_code_layer_horizon_master <- df[, which(names(df) == "code_layer")]} else
    if ("horizon_master" %in% names(df)) {
      column_code_layer_horizon_master <- df[, which(names(df) == "horizon_master")]};
    
    if ("repetition" %in% names(df)) {
      column_repetition_profile_pit_id <- df[, which(names(df) == "repetition")]} else
    if ("profile_pit_id" %in% names(df)) {
      column_repetition_profile_pit_id <- df[, which(names(df) == "profile_pit_id")]};
    ...
    for (j in vec_primary) {
      ...
      { if (any(is.na(df[,j]) |
        ...
        list_primary_inconsistencies <- rbind(list_primary_inconsistencies, 
                                              data.frame(survey_form = (rep(survey_form,length(vec_inconsistency))), 
                                                         ...
                                                         code_layer_horizon_master = column_code_layer_horizon_master[vec_inconsistency],
                                                         repetition_profile_pit_id = column_repetition_profile_pit_id[vec_inconsistency],
                                                         code_line = df$code_line[vec_inconsistency],
                                                         parameter = (rep("survey_year",length(vec_inconsistency))), 
        ...
        if (any((df[vec_nonempty,j] < year_min) | (df[vec_nonempty,j] > year_max))) {
      ...
      { if (any(is.na(df[,j]) |
        ...
        list_primary_inconsistencies <- rbind(list_primary_inconsistencies, 
                                              data.frame(survey_form = (rep(survey_form,length(vec_inconsistency))), 
                                                         ...
                                                         repetition_profile_pit_id = column_repetition_profile_pit_id[vec_inconsistency],
                                                         code_line = df$code_line[vec_inconsistency],
                                                         parameter = (rep("horizon_master",length(vec_inconsistency))), 
                                                         parameter_unit = (rep(NA,length(vec_inconsistency))),
      ...
      { if ("code_layer_original" %in% names(df)) {
        col <- which(names(df) == "code_layer_original")} else {
        col <- j};
        ...
        list_primary_inconsistencies <- rbind(list_primary_inconsistencies, 
                                              data.frame(survey_form = (rep(survey_form,length(vec_inconsistency))), 
                                                         ...
                                                         parameter_unit = (rep(NA,length(vec_inconsistency))),
                                                         parameter_value = column_code_layer_horizon_master[vec_inconsistency],
                                                         inconsistency_reason = inconsistency_reason,
                                                         inconsistency_type = inconsistency_type,
                                                         rule_ID = rule_ID,
                                                         non_duplicated_error_type_per_record = rep(TRUE,length(vec_inconsistency)),
      ...
      { if (any(is.na(df[,j]) |
        ...
        list_primary_inconsistencies <- rbind(list_primary_inconsistencies, 
                                              data.frame(survey_form = (rep(survey_form,length(vec_inconsistency))), 
                                                         ...
                                                         repetition_profile_pit_id = column_repetition_profile_pit_id[vec_inconsistency],
                                                         code_line = df$code_line[vec_inconsistency],
                                                         parameter = (rep("profile_pit_id",length(vec_inconsistency))), 
                                                         parameter_unit = (rep(NA,length(vec_inconsistency))),
  ...
  if (length(unlist(strsplit(survey_form_as_input,"_"))) > 1) {
    assign(paste0("list_primary_inconsistencies_", survey_form_as_input), 
           list_primary_inconsistencies, envir=globalenv())} else {
