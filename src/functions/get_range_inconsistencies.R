...
#' (even though they have not always been mandatory to report) reported?
...
#'  "horizon_gypsum") or with a factor *1000 (g cm-3 to kg m-3 for "bulk_density",
...
get_range_inconsistencies <- function(survey_form, solve = NULL) {
  ...
  # For survey forms: "s1_som", "s1_pfh", "s1_prf", "s1_pls", "y1_st1",
  #                   "so_som", "so_pfh", "so_prf", "so_pls", "si_sta",
  #                   "sw_swc"
  ...
  # e.g. many analytical values can't be negative; fractions/percentages can often 
  # not be higher than 100 %; bulk densities can't be higher than 2650 kg m-3 
  # (mineral density of quartz)...
  ...
  if (unlist(strsplit(survey_form, "_"))[2] %in% c("som", "prf", "pfh")) {
    ...
    parameters_mandatory <- 
      read.csv("./data/additional_data/parameters_mandatory.csv", sep=";", dec = ",")
    
    parameters_mandatory_df <- parameters_mandatory[
      which((parameters_mandatory$survey_form == unlist(strsplit(survey_form, "_"))[2]) &
  ...
  } else {length_parameters_mandatory <- 0}
  ...
  parameters_wrong_units <- names(df)[which(names(df) %in% 
                                              c("bulk_density", "organic_carbon_total",
                                                "n_total", "horizon_bulk_dens_measure",
                                                "horizon_bulk_dens_est", "horizon_c_organic_total",
                                                "horizon_n_total", "horizon_gypsum",
                                                "horizon_caco3_total"))];
  length_parameters_wrong_units <- length(parameters_wrong_units)
  } else {length_parameters_wrong_units <- 0}
  ...
  if (unlist(strsplit(survey_form, "_"))[2] %in% c("som", "pfh", "swc")) {
      ...
      {names(ranges_qaqc)[which(names(ranges_qaqc) == "parameter_pfh")] <- "parameter"} else
        
      if (unlist(strsplit(survey_form, "_"))[2] == "som")
      {names(ranges_qaqc)[which(names(ranges_qaqc) == "parameter_som")] <- "parameter"} else
        
      if (survey_form == "sw_swc")
      {names(ranges_qaqc)[which(names(ranges_qaqc) == "parameter_som")] <- "parameter"} 
  
  parameters <- names(df)[names(df) %in% ranges_qaqc$parameter];
  length_parameters_range <- length(parameters)
  } else {length_parameters_range <- 0}
  
  
  # 1.4 Possible codes
  
  if (unlist(strsplit(survey_form, "_"))[2] %in% c("som", "pfh", "prf", "pls")) {
  ...
  subdir <- list.dirs(path = dir, 
    ...
    str_subset(.,pattern = "\\d$") # selects the string that ends in a number (i.e. not the subfolders)
  
  d_texture_class <- 
    read.csv(paste0(subdir,"/adds/dictionaries/d_texture_class.csv"), sep=";")
  d_soil_group <- 
    read.csv(paste0(subdir,"/adds/dictionaries/d_soil_group.csv"), sep=";")
  d_soil_adjective <- 
    read.csv(paste0(subdir,"/adds/dictionaries/d_soil_adjective.csv"), sep=";")
  d_soil_specifier <- 
    read.csv(paste0(subdir,"/adds/dictionaries/d_soil_specifier.csv"), sep=";")
  d_wrb_pub <- 
    read.csv(paste0(subdir,"/adds/dictionaries/d_wrb_pub.csv"), sep=";")
  d_parent_material <- 
    read.csv(paste0(subdir,"/adds/dictionaries/d_parent_material.csv"), sep=";")
  d_hori_disc <- 
    read.csv(paste0(subdir,"/adds/dictionaries/d_hori_disc.csv"), sep=";")
  ...
  } else {length_parameters_code <- 0}
  
  
  # Set up a progress bar to track processing
  
  if (unlist(strsplit(survey_form, "_"))[2] %in% c("som", "pfh", "prf", "pls", "swc")) {
  progress_bar <- txtProgressBar(min = 0, 
                  max = (length_parameters_mandatory + length_parameters_wrong_units + 
  ...
  list_range_inconsistencies <- data.frame(survey_form = NULL, 
                                           partner = NULL, 
                                           partner_code = NULL, 
                                           country = NULL,
                                           code_country = NULL, 
  ...
  if (unlist(strsplit(survey_form, "_"))[2] == "som") {
    ...
    for (i in 1:nrow(df)) {
      ...
      if (any(!is.na(df$coarse_fragment_vol[vec]) &
              (df$coarse_fragment_vol[vec] != ""))) {df$coarse_fragment_vol_any_survey[i] <- TRUE};
      
      # Has bulk density ever been reported?
      
      if (any(!is.na(df$bulk_density[vec]) &
              (df$bulk_density[vec] != ""))) {df$bulk_density_any_survey[i] <- TRUE};
      
      # Have the clay, silt and sand contents ever been reported?
      
      if (any(!is.na(df$part_size_clay[vec]) &
              (df$part_size_clay[vec] != ""))) {df$part_size_clay_any_survey[i] <- TRUE};
      if (any(!is.na(df$part_size_silt[vec]) &
              (df$part_size_silt[vec] != ""))) {df$part_size_silt_any_survey[i] <- TRUE};
      if (any(!is.na(df$part_size_sand[vec]) &
              (df$part_size_sand[vec] != ""))) {df$part_size_sand_any_survey[i] <- TRUE};
      
      # Has "exch_ca" ever been reported?
      
      if (any(!is.na(df$exch_ca[vec]) &
              (df$exch_ca[vec] != ""))) {df$exch_ca_any_survey[i] <- TRUE};
      
      # Have "rea_al" and "rea_fe" ever been reported?
      
      if (any(!is.na(df$rea_al[vec]) &
              (df$rea_al[vec] != ""))) {df$rea_al_any_survey[i] <- TRUE};
      if (any(!is.na(df$rea_fe[vec]) &
              (df$rea_fe[vec] != ""))) {df$rea_fe_any_survey[i] <- TRUE};
      ...
      if ((!is.na(df$ph_cacl2[i]) &
           (df$ph_cacl2[i] != "")) & 
          (df$ph_cacl2[i] > 5.5)) {df$calcareous_pH5[i] <- TRUE};
      
      # Is the pH_CaCl2 higher than 6?
      
      if ((!is.na(df$ph_cacl2[i]) &
         (df$ph_cacl2[i] != "")) & 
        (df$ph_cacl2[i] > 6)) {df$calcareous_pH6[i] <- TRUE}}
    
  }
  
  # For each of the parameters for which this has to be tested
  
  for (i in 1:length(parameters_mandatory_names)) {
    ...
    if (start_year < 2004) {start_year <- 2004}
 ...
 # "som"
    ...
    if (unlist(strsplit(survey_form, "_"))[2] == "som") {
      ...
      if (unlist(strsplit(survey_form, "_"))[1] == "s1") {
        layers_mandatory <- layers_mandatory[which(str_detect(layers_mandatory, "^LI_"))]} else
          
      if (unlist(strsplit(survey_form, "_"))[1] == "so") {
        layers_mandatory <- layers_mandatory[which(str_detect(layers_mandatory, "^LII_"))]};
      
      # Check if the parameter is still mandatory for any layers
      
      if (!identical(layers_mandatory, character(0))) {
      
      # Use the "parameters_layers" table to check the exact code_layers and depths
      # of the mandatory layers
      ...
      layer_limit_upper <- min(parameters_layers_mandatory$layer_limit_superior);
      layer_limit_lower <- max(parameters_layers_mandatory$layer_limit_inferior);
      ...
      if ("OF" %in% parameters_layers_mandatory$code_layer |
        ...
        vec_inconsistency <- 
          vec_inconsistency[which((substr(df$code_layer[vec_inconsistency],1,2) %in% c("OF", "OH")) |
                                    (substr(df$code_layer[vec_inconsistency],1,1) == "H") |
                                    (df$code_layer[vec_inconsistency] %in% 
                                       parameters_layers_mandatory$code_layer) |
                                    ((df$layer_limit_superior[vec_inconsistency] > layer_limit_upper) &
                                       (df$layer_limit_inferior[vec_inconsistency] < 
                                          layer_limit_lower)))]} else
      ...
      # Per parameter, finetune "vec_inconsistencies" based on the special conditions
      # using the extra columns that were added to "df"
      # (e.g. some parameters only have to be reported once, are only mandatory in
      # non-stony layers, etc), to select the exact row indices for which the
      # mandatory parameter is not reported
      ...
      if (parameters_mandatory_names[i] == "coarse_fragment_vol") {
        vec_inconsistency <- vec_inconsistency[
          which(is.na(df$coarse_fragment_vol_any_survey[vec_inconsistency]) |
                  (df$coarse_fragment_vol_any_survey[vec_inconsistency] == ""))]} else
      ...
      if (parameters_mandatory_names[i] == "exch_ca") {
        vec_inconsistency <- vec_inconsistency[
          which(((is.na(df$exch_ca[vec_inconsistency]) |
               ...
               (!df$layer_type[vec_inconsistency] %in% c("peat", "forest_floor"))) |
            ...
            ((is.na(df$exch_ca[vec_inconsistency]) |
               ...
               (df$layer_type[vec_inconsistency] == "peat" |
                  df$code_layer[vec_inconsistency] %in% c("OL", "OF"))))]} else
        ...
        vec_inconsistency <- vec_inconsistency[
          which(((is.na(df$rea_al[vec_inconsistency]) |
            ...
            (is.na(df$rea_al_any_survey[vec_inconsistency]) &
               (df$code_layer[vec_inconsistency] %in% c("M24", "M48"))))]} else
    ...
    {vec_inconsistency <- vec_inconsistency[
       which(is.na(df[vec_inconsistency,which(names(df) == parameters_mandatory_names[i])]) |
      ...
      if ("code_layer_original" %in% names(df)) {
        ind_layer_horizon <- as.character(df$code_layer_original[vec_inconsistency])} else
        {ind_layer_horizon <- as.character(df$code_layer[vec_inconsistency])}
      
     # ind_layer_horizon <- df$code_layer[vec_inconsistency];
 ...
 # "prf"
    ...
    if (unlist(strsplit(survey_form, "_"))[2] == "prf") {
      ...
      # Select the row indices for which the given parameter is mandatory but absent
 ...
 # "pfh"
    ...
    vec_data <- df[vec_inconsistency,which(names(df) == parameters_mandatory_names[i])]
    ...
    # If it concerns soil classification-related parameters: mention that the parameters
    # should ideally be reported according to WRB 2006 or 2007
    
    if ((unlist(strsplit(survey_form, "_"))[2] == "prf") &
        (parameters_mandatory_names[i] %in% 
         c("code_wrb_soil_group", "code_wrb_qualifier_1", "code_wrb_publication"))) {
      
      inconsistency_reason <- paste0(inconsistency_reason," ",start_year,
         ". Preferably, the soil classification should be reported according to the World Reference Base (WRB) version of either 2006 or 2007.")
      
    } else {inconsistency_reason <- paste0(inconsistency_reason," ",start_year,".")} 
    
    inconsistency_type <- 
      inconsistency_catalogue$inconsistency_type[
        which(inconsistency_catalogue$rule_ID == rule_ID)];
    
    list_range_inconsistencies <- rbind(list_range_inconsistencies, 
                                        data.frame(survey_form = as.factor(rep(survey_form,length(vec_inconsistency))), 
                                                   ...
                                                   parameter = as.factor(rep(parameters_mandatory_names[i],length(vec_inconsistency))), 
                                                   parameter_unit = rep(parameter_unit,length(vec_inconsistency)),
  ...
  if (unlist(strsplit(survey_form, "_"))[2] == "prf") {
    ...
    # Identify these "informative" parameters based on the "parameters_mandatory" table
    ...
    for (i in 1:length(parameters_informative)) {
      ...
      inconsistency_reason <- paste0(inconsistency_reason[1], start_year, inconsistency_reason[2])
      
      # If it concerns soil classification-related parameters: mention that the parameters
      # should ideally be reported according to WRB 2006 or 2007
      
      if (parameters_informative[i] %in% 
          c("code_wrb_soil_group", "code_wrb_qualifier_1", "code_wrb_publication")) {
      ...
      list_range_inconsistencies <- rbind(list_range_inconsistencies, 
                                          data.frame(survey_form = as.factor(rep(survey_form,length(vec_inconsistency))), 
                                                     ...
                                                     code_plot = df$code_plot[vec_inconsistency], 
                                                     plot_ID = df$plot_ID[vec_inconsistency],
                                                     code_layer_horizon_master = rep(NA,length(vec_inconsistency)),
                                                     repetition_profile_pit_id = df$profile_pit_id[vec_inconsistency],
                                                     code_line = df$code_line[vec_inconsistency],
                                                     parameter = as.factor(rep(parameters_informative[i],length(vec_inconsistency))), 
                                                     parameter_unit = rep(parameter_unit,length(vec_inconsistency)),
  ...
  if (unlist(strsplit(survey_form, "_"))[2] == "prf") {
    ...
    list_range_inconsistencies <- rbind(list_range_inconsistencies, 
                                        data.frame(survey_form = (rep(survey_form,length(j))), 
                                                   ...
                                                   parameter_value = as.character(df[j,which(names(df) == "code_wrb_publication")]),
                                                   inconsistency_reason = inconsistency_reason,
                                                   inconsistency_type = inconsistency_type,
                                                   rule_ID = rule_ID,
    ...
    list_range_inconsistencies <- rbind(list_range_inconsistencies, 
                                        data.frame(survey_form = (rep(survey_form,length(j))), 
                                                   ...
                                                   parameter_unit = rep("-",length(j)),
                                                   parameter_value = as.character(df[j,which(names(df) == "code_wrb_soil_group")]),
                                                   inconsistency_reason = inconsistency_reason,
                                                   inconsistency_type = inconsistency_type,
                                                   rule_ID = rule_ID,
    ...
    list_range_inconsistencies <- rbind(list_range_inconsistencies, 
                                        data.frame(survey_form = (rep(survey_form,length(j))), 
                                                   ...
                                                   parameter_unit = rep("-",length(j)),
                                                   parameter_value = as.character(df[j,which(names(df) == "code_wrb_qualifier_1")]),
                                                   inconsistency_reason = inconsistency_reason,
                                                   inconsistency_type = inconsistency_type,
                                                   rule_ID = rule_ID,
                                                   non_duplicated_error_type_per_record = rep(FALSE,length(j)),
  ...
  if (survey_form %in% c("si_sta", "y1_st1")) {
    ...
    if (survey_form == "y1_st1") {
      data_availability_level <- get("data_availability_s1", envir = .GlobalEnv)} else
    ...
    data_availability_plots <- data_availability_plots[!duplicated(data_availability_plots$plot_ID),
    ...
    for (j in seq_along(data_availability_plots$plot_ID)) {
      ...
      # Retrieve the row indices of the records of the given plot j in the given
      # system installment survey form
      
      vec_forest_type <- which(df$plot_ID == data_availability_plots$plot_ID[j])
      ...
      # If any forest type has been reported: store the unique "code_forest_type"
      # code(s) which have been reported for the given plot
    ...
    vec_inconsistency <- which(data_availability_plots$code_forest_type_count == 0)
    
    # Store information about the inconsistency in "list_range_inconsistencies"
    
    # This is actually a general list with plots for which code_forest_type is lacking
    ...
    # (since it can be filled in in either of these plot records)
      ...
      list_range_inconsistencies <- rbind(list_range_inconsistencies, 
                                          data.frame(survey_form = (rep(survey_form,length(vec_inconsistency))), 
                                                     ...
                                                     partner_code = data_availability_plots$partner_code[vec_inconsistency], 
                                                     country = data_availability_plots$country[vec_inconsistency],
                                                     code_country = data_availability_plots$code_country[vec_inconsistency], 
                                                     survey_year = rep(NA, length(vec_inconsistency)),
                                                     code_plot = data_availability_plots$code_plot[vec_inconsistency], 
                                                     plot_ID = data_availability_plots$plot_ID[vec_inconsistency],
                                                     code_layer_horizon_master = rep(NA, length(vec_inconsistency)),
                                                     repetition_profile_pit_id = rep(NA, length(vec_inconsistency)),
                                                     code_line = rep(NA, length(vec_inconsistency)), 
                                                     parameter = (rep("code_forest_type",length(vec_inconsistency))), 
  ...
  # Because of the assumption that, when a partner is submitting data for a certain
  # survey year, the data of a certain parameter are all reported in the same units
  # So if a mistake in units has been made, it should be the case for all data
  # of the given "unique_partner_survey"

  df$unique_partner_survey <- paste0(df$partner_code, "_", df$survey_year)
  
  # For parameter for which units can be potentially wrong:

  for (i in 1:length(parameters_wrong_units)) {
    ...
    # the information whether a given observation was reported in the wrong unit.
    
    if (parameters_wrong_units[i] %in% 
        c("bulk_density", "horizon_bulk_dens_measure")) {
      df$bulk_density_wrong_unit <- NA;
      wrong_range_org <- c(0, 2.65);
      wrong_range_mineral <- c(0, 2.65)} else # g cm-3
    ...
    for (j in 1:length(unique(df$unique_partner_survey))) {
      
      # Determine the row indices, data and layer types of the records which do
      # contain data for the given parameter in the given "unique_partner_survey"
      
      vec <- which(df$unique_partner_survey == unique(df$unique_partner_survey)[j]);
      ...
      if (!identical(vec_data, logical(0))) {
        ...
        # Check for each layer/matrix type (mineral versus organic) whether at least
        # 90 % of the data for the given unique_partner_survey are within the plausible
        # range with the wrong units.
        ...
        if ((length(which((vec_layer_type != "mineral" &
                  ...
                  (vec_layer_type == "mineral" &
                     ...
                     vec_data < wrong_range_mineral[2]))) >= 0.9 * length(vec_data)) &
            length(vec_data) >= 2) {
        
          # Store the information whether a given observation was reported in the
          # wrong unit in "df" (TRUE)
          # This is done so that the "plausible range" test (FSCC_14) would exclude
          # these values (since they are anyway likely to be outside the plausible
          # range)
    
          if (parameters_wrong_units[i] %in% 
              c("bulk_density", "horizon_bulk_dens_measure")) {
            df$bulk_density_wrong_unit[vec_nonempty] <- TRUE} else
              
          if (parameters_wrong_units[i] == "horizon_bulk_dens_est") {
            df$bulk_density_est_wrong_unit[vec_nonempty] <- TRUE} else
              
          if (parameters_wrong_units[i] %in%
              c("organic_carbon_total", "horizon_c_organic_total")) {
            df$organic_carbon_wrong_unit[vec_nonempty] <- TRUE} else
              
          if (parameters_wrong_units[i] %in% c("n_total", "horizon_n_total")) {
            df$total_nitrogen_wrong_unit[vec_nonempty] <- TRUE} else
              
          if (parameters_wrong_units[i] == "horizon_caco3_total") {
            df$caco3_wrong_unit[vec_nonempty] <- TRUE} else
          ...
          ind_repetition_profile_pit_id <- df$profile_pit_id[vec_nonempty]} else
            
          if (unlist(strsplit(survey_form, "_"))[2] == "som") 
          {if ("code_layer_original" %in% names(df)) {
            ind_layer_horizon <- as.character(df$code_layer_original[vec_nonempty])} else
            {ind_layer_horizon <- as.character(df$code_layer[vec_nonempty])}
           # ind_layer_horizon <- df$code_layer[vec_nonempty];
          ind_repetition_profile_pit_id <- df$repetition[vec_nonempty]} else
          ...
          if (parameters_wrong_units[i] %in% 
              c("bulk_density", "horizon_bulk_dens_measure", "horizon_bulk_dens_est")) {
            
          ind_parameter_units <- as.factor(rep("kg m-3", length(vec_nonempty)));
          inconsistency_reason <- paste0(inconsistency_reason, 
                  " (i.e. reported in g cm-3 instead of kg m-3).")} else
                    
          if (parameters_wrong_units[i] %in%
              c("organic_carbon_total", "n_total", "horizon_c_organic_total",
                "horizon_n_total", "horizon_caco3_total", "horizon_gypsum")) {
          ...
          list_range_inconsistencies <- rbind(list_range_inconsistencies, 
                                              data.frame(survey_form = as.factor(rep(survey_form,length(vec_nonempty))), 
                                                         ...
                                                         parameter = as.factor(rep(parameters_wrong_units[i],length(vec_nonempty))), 
                                                         parameter_unit = ind_parameter_units,
                                                         parameter_value = vec_data,
                                                         inconsistency_reason = inconsistency_reason,
                                                         inconsistency_type = inconsistency_type,
                                                         rule_ID = rule_ID,
                                                         non_duplicated_error_type_per_record = rep(TRUE,length(vec_nonempty)),
                                                         change_date = df$change_date[vec_nonempty],
                                                         download_date = rep(download_date_pir,length(vec_nonempty))))
  ...
  # 4. FSCC 38 and FSCC 14: Check whether values are in possible or plausible range
  ...
  for (i in 1:length(parameters)) {
    ...
    # If no possible range has been provided, consider it possible to be "infinite"
    ...
    if (is.na(range_min_possible)) {range_min_possible <- (-1e99)};
    if (is.na(range_max_possible)) {range_max_possible <- 1e99};
    
    # For each of the records of the given parameter column
    
    for (j in 1:length(df$active_column)) {
      
      # Assign plausible ranges for the given parameter and layer, based on its layer type
    ...
    if (unlist(strsplit(survey_form, "_"))[2] == "som") {
        ...
        # The plausible range of layer "M05" - which is located at the transition
        # between forest floor and mineral, and therefore still contains a 
        # considerable influence by characteristics of organic layers - is wider than
        # the plausible ranges of mineral and organic matrices
        # for the parameters "bulk density", "organic_carbon_total" and "n_total".
      
        if  (df$code_layer[j] == "M05" & 
             (column_name %in% c("bulk_density", "organic_carbon_total", "n_total"))) {
          range_min <- min(ranges_qaqc$min_org[which(ranges_qaqc$parameter == parameters[i])],
                           ranges_qaqc$min_mineral[which(ranges_qaqc$parameter == parameters[i])]);
          range_max <- max(ranges_qaqc$max_org[which(ranges_qaqc$parameter == parameters[i])],
                           ranges_qaqc$max_mineral[which(ranges_qaqc$parameter == parameters[i])])} else
        
        # If the given layer is mineral: assign plausible ranges for mineral matrices
        # of the given parameter
                                 
        if (df$layer_type[j] == "mineral") 
        {range_min <- ranges_qaqc$min_mineral[which(ranges_qaqc$parameter == parameters[i])];
        range_max <- ranges_qaqc$max_mineral[which(ranges_qaqc$parameter == parameters[i])]} else
      
        # If the given layer is organic: assign plausible ranges for organic matrices
        # of the given parameter  
          
        if ((df$layer_type[j] == "forest_floor" | df$layer_type[j] == "peat"))
          {range_min <- ranges_qaqc$min_org[which(ranges_qaqc$parameter == parameters[i])];
          range_max <- ranges_qaqc$max_org[which(ranges_qaqc$parameter == parameters[i])]}};
      ...
      if (unlist(strsplit(survey_form, "_"))[2] == "pfh") {
        ...
        # If the given layer is mineral and the parameter is not "horizon_ph" (see below why):
        ...
        range_max <- ranges_qaqc$max_mineral[which(ranges_qaqc$parameter == parameters[i])]} else
          ...
          # If the given layer is organic and the parameter is not "horizon_ph" (see below why): 
          # assign plausible ranges for organic matrices of the given parameter
          
          if ((df$layer_type[j] == "forest_floor" | df$layer_type[j] == "peat") & 
              column_name != "horizon_ph")
          {range_min <- ranges_qaqc$min_org[which(ranges_qaqc$parameter == parameters[i])];
          range_max <- ranges_qaqc$max_org[which(ranges_qaqc$parameter == parameters[i])]} else
         
        # This is stated in the manual for the parameter "horizon_ph" in "pfh" survey forms: 
        # "Please specify in “other_observations” if distilled water or KCl was used"
        # Hence, we need to figure out based on the column "other_obs", whether the
        # plausible ranges for pH_H2O versus pH_CaCl2 apply for the column "horizon_ph"
        ...
        # as pH_H2O, and assign the corresponding row index in the "ranges_qaqc" dataframe
          
        {if (str_detect(df$other_obs[j],"H2O|H20|water"))
        {ind_pH <- which(ranges_qaqc$parameter_som == "ph_h2o")} else
          
          # If a reference to CaCl2 can be found in this column, consider the pH
          # as pH_CaCl2, and assign the corresponding row index in the "ranges_qaqc" dataframe
          ...
          range_max <- ranges_qaqc$max_mineral[ind_pH]} else
    ...
    # Error: outside possible range  
      ...
      if (!is.na(range_min_possible) & !is.na(range_max_possible) & 
        ...
        if (df$active_column[j] != -1 &
          ...
          ind_repetition_profile_pit_id <- df$profile_pit_id[j]} else
            
          if (unlist(strsplit(survey_form, "_"))[2] == "som") 
          {if ("code_layer_original" %in% names(df)) {
            ind_layer_horizon <- as.character(df$code_layer_original[j])} else
            {ind_layer_horizon <- as.character(df$code_layer[j])}
          #  ind_layer_horizon <- df$code_layer[j];
          ind_repetition_profile_pit_id <- df$repetition[j]} else
          ...
          list_range_inconsistencies <- rbind(list_range_inconsistencies, 
                                              data.frame(survey_form = as.factor(rep(survey_form,length(j))), 
                                                         ...
                                                         parameter_unit = as.factor(rep(ranges_qaqc$unit[which(parameters[i] == ranges_qaqc$parameter)],length(j))),
     ...
     if (!is.na(range_min) & !is.na(range_max) & !is.na(df$active_column[j]))
     ...
     {if (df$active_column[j] != -1 & 
          (df$active_column[j] < range_min | df$active_column[j] > range_max) &
          df$active_column[j] > range_min_possible &
          df$active_column[j] < range_max_possible)
       
     # Check whether the parameter value was possibly reported in the wrong units
     # because we can ignore these data for now
       
     { j_wrong_units <- NA;
       if (column_name %in% c("bulk_density", "horizon_bulk_dens_measure")) {
         j_wrong_units <- df$bulk_density_wrong_unit[j]} else
       if (column_name == "horizon_bulk_dens_est") {
         j_wrong_units <- df$bulk_density_est_wrong_unit[j]} else
       if (column_name %in% c("organic_carbon_total", "horizon_c_organic_total")) {
         j_wrong_units <- df$organic_carbon_wrong_unit[j]} else
       if (column_name %in% c("n_total", "horizon_n_total")) {
         j_wrong_units <- df$total_nitrogen_wrong_unit[j]} else
       if (column_name == "horizon_caco3_total") {
         j_wrong_units <- df$caco3_wrong_unit[j]} else
       ...
       list_range_inconsistencies <- rbind(list_range_inconsistencies, 
                                    data.frame(survey_form = as.factor(rep(survey_form,length(j))), 
                                               ...
                                               code_line = df$code_line[j],
                                               parameter = as.factor(rep(parameters[i],length(j))), 
                                               parameter_unit = as.factor(rep(ranges_qaqc$unit[which(parameters[i] == ranges_qaqc$parameter)][1],length(j))),
                                               parameter_value = df$active_column[j],
                                               inconsistency_reason = inconsistency_reason,
                                               inconsistency_type = inconsistency_type,
                                               rule_ID = rule_ID,
      ...
      # If the given column is "horizon_ph", and if a value has been reported while
      # no extraction method has been specified in the "other_obs" column:
      
      if (column_name == "horizon_ph")
      {if (!is.na(df$active_column[j]) &
           (is.na(df$other_obs[j]) | !str_detect(df$other_obs[j], "H2O|H20|water|CaCl2")))
  ...
  for (i in 1:length(parameters_code)) {
    ...
    if (unlist(strsplit(survey_form, "_"))[2] == "som") { 
      ...
      list_range_inconsistencies <- rbind(list_range_inconsistencies, 
                                          data.frame(survey_form = as.factor(rep(survey_form,length(vec_inconsistency))), 
                                                     ...
                                                     repetition_profile_pit_id = ind_repetition_profile_pit_id,
                                                     code_line = df$code_line[vec_inconsistency],
                                                     parameter = as.factor(rep(parameter_code,length(vec_inconsistency))), 
                                                     parameter_unit = rep("-",length(vec_inconsistency)),
                                                     parameter_value = vec_data,
                                                     inconsistency_reason = inconsistency_reason,
                                                     inconsistency_type = inconsistency_type,
    ...
    setTxtProgressBar(progress_bar, (length_parameters_mandatory + 
                      length_parameters_wrong_units + length_parameters_range + i))
  }}
  ...
  # FSCC_46: one of the three texture classes (clay/silt/sand) is missing while the others are not.
  ...
  if (unlist(strsplit(survey_form, "_"))[2] %in% c("som", "pfh")) {
    ...
    # Create a vector with row indices with only two out of three texture fractions
    # reported
    ...
    if (unlist(strsplit(survey_form, "_"))[2] == "som") {
      if ("code_layer_original" %in% names(df)) {
        ind_layer_horizon <- as.character(df$code_layer_original)} else
        {ind_layer_horizon <- as.character(df$code_layer)}
     # ind_layer_horizon <- df$code_layer
  ...
  if (unlist(strsplit(survey_form, "_"))[2] == "pfh") {
    ...
    for (i in 1:length(unique(df$partner_code))) {
      ...
      if (!identical(vec, integer(0))) {
        ...
        # Create just one line per partner in the inconsistency report, and summarise
        # the survey years by providing the range of survey years
        
        change_date_i <- paste0(format(as.Date((min(df$change_date[vec]))),"%Y")," - ",
  ...
  # Save the survey form and list_range_inconsistencies for the given survey form;
  # for the given survey form to the global environment
