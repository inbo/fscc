...
#' - FSCC_45: Does the sum of the reported exchangeable acid cations (Al, Fe, Mn)
#'   and free H+ approximate the reported exchangeable acidity?
#'
...
get_derived_variables <- function(survey_form) {
  ...
  for (i in 1:nrow(df))  {
    ...
    if (!is.na(df$part_size_clay[i]) &
      ...
      # LOQ = 1 %
      if (df$part_size_clay[i] == -1) {clay <- 0.5} else {clay <- df$part_size_clay[i]};
      if (df$part_size_silt[i] == -1) {silt <- 0.5} else {silt <- df$part_size_silt[i]};
      if (df$part_size_sand[i] == -1) {sand <- 0.5} else {sand <- df$part_size_sand[i]};
      df$sum_texture[i] <- clay + silt + sand}
      # check if this is between 97 and 103 %
    ...
    if (!is.na(df$organic_carbon_total[i]) &
      ...
      # check if this is in a plausible range (1 to 100)
    ...
    if (!is.na(df$exch_ca[i]) &
      ...
      # LOQ = 0.03 cmol+ kg-1
      if (df$exch_ca[i] == -1) {exch_ca <- 0.015} else {exch_ca <- df$exch_ca[i]};
      if (df$exch_mg[i] == -1) {exch_mg <- 0.015} else {exch_mg <- df$exch_mg[i]};
      if (df$exch_k[i] == -1) {exch_k <- 0.015} else {exch_k <- df$exch_k[i]};
      if (df$exch_na[i] == -1) {exch_na <- 0.015} else {exch_na <- df$exch_na[i]};
      
      df$sum_base_cations[i] <- exch_ca + exch_mg + exch_k + exch_na}
      # check if this is <= CEC (exch_cec)
    ...
    if (!is.na(df$exch_al[i]) &
      ...
      # LOQ = 0.02 cmol+ kg-1
      if (df$exch_al[i] == -1) {exch_al <- 0.01} else {exch_al <- df$exch_al[i]};
      if (df$exch_fe[i] == -1) {exch_fe <- 0.01} else {exch_fe <- df$exch_fe[i]};
      if (df$exch_mn[i] == -1) {exch_mn <- 0.01} else {exch_mn <- df$exch_mn[i]};
      # LOQ = 0.1 cmol+ kg-1
      if (df$free_h[i] == -1) {free_h <- 0.05} else {free_h <- df$free_h[i]};
      
      df$sum_acid_cations[i] <- exch_al + exch_fe + exch_mn + free_h}
      # check if this approximates the exchangeable acidity (exch_acidiy)
  }} 
  
  
  # vec <- which(!is.na(df$sum_base_cations) & !is.na(df$exch_cec))
  # if (!identical(vec,integer(0)))
  # {plot(df$sum_base_cations[vec],df$exch_cec[vec])}
  # 
  # vec <- which(!is.na(df$sum_base_cations) & !is.na(df$exch_bce))
  # if (!identical(vec,integer(0)))
  # {plot(df$sum_base_cations[vec],df$exch_bce[vec])}
  # 
  # vec <- which(!is.na(df$sum_acid_cations) & !is.na(df$exch_cec))
  # if (!identical(vec,integer(0)))
  # {plot(df$sum_acid_cations[vec],df$exch_cec[vec])}
  # 
  # vec <- which(!is.na(df$sum_acid_cations) & !is.na(df$exch_ace))
  # if (!identical(vec,integer(0)))
  # {plot(df$sum_acid_cations[vec],df$exch_ace[vec])}
  # 
  # vec <- which(!is.na(df$sum_acid_cations) & !is.na(df$sum_base_cations))
  # if (!identical(vec,integer(0)))
  # {plot(df$sum_acid_cations[vec],df$sum_base_cations[vec])}
  # 
  # vec <- which(!is.na(df$exch_bce) & !is.na(df$exch_ace))
  # if (!identical(vec,integer(0)))
  # {plot(df$exch_bce[vec],df$exch_ace[vec])}
  ...
  if (unlist(strsplit(survey_form, "_"))[2] == "pfh") {
    ...
    for (i in 1:nrow(df)) {
      ...
      if (!is.na(df$horizon_clay[i]) &
        ...
        # LOQ = 1 %
        if (df$horizon_clay[i] == -1) {clay <- 0.5} else {clay <- df$horizon_clay[i]};
        if (df$horizon_silt[i] == -1) {silt <- 0.5} else {silt <- df$horizon_silt[i]};
        if (df$horizon_sand[i] == -1) {sand <- 0.5} else {sand <- df$horizon_sand[i]};
        df$sum_texture[i] <- clay + silt + sand}
      ...
      if (!is.na(df$horizon_exch_ca[i]) &
        ...
        # LOQ = 0.03 cmol+ kg-1
        if (df$horizon_exch_ca[i] == -1) {exch_ca <- 0.015} else {
          exch_ca <- df$horizon_exch_ca[i]};
        if (df$horizon_exch_mg[i] == -1) {exch_mg <- 0.015} else {
          exch_mg <- df$horizon_exch_mg[i]};
        if (df$horizon_exch_k[i] == -1) {exch_k <- 0.015} else {
          exch_k <- df$horizon_exch_k[i]};
        if (df$horizon_exch_na[i] == -1) {exch_na <- 0.015} else {
  ...
  # vec <- which(!is.na(df$sum_base_cations) & !is.na(df$horizon_cec))
  # if (!identical(vec,integer(0)))
  # {plot(df$sum_base_cations[vec],df$horizon_cec[vec])}
  
  length(which(!is.na(df$bulk_density_layer_weight)))
  length(which(!is.na(df$sum_texture)))
  length(which(!is.na(df$c_to_n_ratio)))
  length(which(!is.na(df$sum_base_cations)))
  length(which(!is.na(df$sum_acid_cations)))
  ...
  list_derived_inconsistencies <- data.frame(survey_form = NULL, 
                                           partner = NULL, 
                                           partner_code = NULL, 
                                           country = NULL,
                                           code_country = NULL, 
  ...
  if ("bulk_density_layer_weight" %in% names(df)) {
    ...
    if (!identical(vec_inconsistency, integer(0))) {
      ...
      list_derived_inconsistencies <- rbind(list_derived_inconsistencies, 
                                          data.frame(survey_form = as.factor(rep(survey_form,length(i))), 
                                                     ...
                                                     parameter = as.factor(rep("organic_layer_weight",length(i))), 
  ...
  if ("sum_base_cations" %in% names(df)) {
    ...
    if (unlist(strsplit(survey_form, "_"))[2] == "som") {cec <- df$exch_cec}
    if (unlist(strsplit(survey_form, "_"))[2] == "pfh") {cec <- df$horizon_cec}
    ...
    vec_inconsistency <- vec_inconsistency[which(
          (df$sum_base_cations[vec_inconsistency] > range_max[vec_inconsistency]) &
            range_max[vec_inconsistency] != -1)]}
    
    if (!identical(vec_inconsistency, integer(0))) {
      
      if (unlist(strsplit(survey_form, "_"))[2] == "som") { # Ca, Mg, K, Na
        if ("code_layer_original" %in% names(df)) {
          ind_layer_horizon <- as.character(df$code_layer_original)} else
          {ind_layer_horizon <- as.character(df$code_layer)}
        ind_repetition_profile_pit_id <- df$repetition;
  ...
  if ("sum_acid_cations" %in% names(df)) {
    ...
    vec_inconsistency <- which(!is.na(df$sum_acid_cations) &
                                 ...
                                 (df$exch_acidiy != -1))
    range_min <- (-2) + df$exch_acidiy[vec_inconsistency] * 0.7 # (based on plot)
    range_max <- 2 + df$exch_acidiy[vec_inconsistency] * 1.3 # (based on plot)
    
    # plot(df$exch_acidiy[vec_inconsistency],df$sum_acid_cations[vec_inconsistency])
    # fit <- lm(df$exch_acidiy[vec_inconsistency]~df$sum_acid_cations[vec_inconsistency])
    # abline(a = coef(fit)[1], b = coef(fit)[2], col = "red")
    # 
    # fit_up <- lm((2+1.2*df$exch_acidiy[vec_inconsistency])~df$sum_acid_cations[vec_inconsistency])
    # abline(a = coef(fit_up)[1], b = coef(fit_up)[2], col = "blue")
    # 
    # fit_low <- lm((-2+0.8*df$exch_acidiy[vec_inconsistency])~df$sum_acid_cations[vec_inconsistency])
    ...
    if (!identical(vec_inconsistency, integer(0))) {
      ...
      if (unlist(strsplit(survey_form, "_"))[2] == "som") {# Al, Fe, Mn, free H+
        if ("code_layer_original" %in% names(df)) {
          ind_layer_horizon <- as.character(df$code_layer_original)} else
          {ind_layer_horizon <- as.character(df$code_layer)}
        ind_repetition_profile_pit_id <- df$repetition};
  ...
  assign(paste0("list_derived_inconsistencies_", survey_form),
         list_derived_inconsistencies, envir=globalenv())
