...
gapfill_internally2 <- function(survey_form,
                               data_frame = NULL,
                               save_to_env = FALSE) {
  ...
  if (unlist(strsplit(survey_form, "_"))[2] == "som") {
    ...
    if (unlist(strsplit(survey_form, "_"))[1] == "so") {
      ...
      name_swc_fixed <- "sw_swc_fixed"

      if (!exists(name_swc_fixed) ||
          (exists(name_swc_fixed) &&
           any(!"data.frame" %in% class(get_env(name_swc_fixed))))) {

        source("./src/functions/harmonise_into_fixed_depth_layers.R")

        swc_fixed <-
          harmonise_into_fixed_depth_layers(data_frame = sw_swc,
                                            survey_form_df = "sw_swc")

        assign_env(name_swc_fixed,
                   swc_fixed)

      } else {

        swc_fixed <- get_env(name_swc_fixed)

      }

      swc_fixed <- swc_fixed %>%
        mutate(code_layer = ifelse(code_layer == "O10",
                                   "O",
                                   code_layer)) %>%
      ...
      swc_fixed_otheryear <- swc_fixed %>%
        group_by(unique_layer) %>%
        summarise(bulk_density =
                    round(mean(bulk_density, na.rm = TRUE), 2),
                  .groups = "drop")
    }
