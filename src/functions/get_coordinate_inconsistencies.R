...
#'
#' @param surveys_with_coordinates Character vector - Vector with names of the survey
#' forms which contain coordinates. For example:
#' c("y1_pl1", "s1_pls", "s1_prf", "si_plt", "so_pls", "so_prf")
#' These data forms must contain the columns "latitude_error" and "longitude_error",
#' which means that they need to be previously processed with the function
#' "dec_coordinates()". 
#' 
#' @param boundary_buffer_meter Numeric - Buffer distance in meters to be added outside
#' the reported partner boundaries as a margin of error for the check if coordinates
#' are within the partner boundaries. Default is 2000 meter.
#' 
#' @details 
...
#'   "latitude_error" and "longitude_error". The function "dec_coordinate()" adds
#'   these columns when converting the coordinates in +/-DDMMSS to decimal degrees,
#'   which is done in the function "read_icpforests_csv()".
#'   
#' - FSCC_13: Are the reported coordinates located within their respective partner
#'   boundaries?
#'   
...
#'   * German Länder: https://maps.princeton.edu/catalog/tufts-germany-states-15
#'   * Flanders and Wallonia: https://hub.arcgis.com/datasets/esribeluxdata::belgium-provinces-1/explore
#'   * Canary islands: https://data.metabolismofcities.org/dashboards/madrid/maps/35475/view/
#'   * Azores: https://data.metabolismofcities.org/dashboards/lisbon/maps/35879/
#'   
#'   Note: Cross-checking border-region coordinates with Google maps reveals that
#'   the borders in the geopackage are not optimally accurate. Therefore, it is
#'   recommended to specify a buffer zone of 2000 meters around the partner boundaries
#'   as an error margin in the "boundary_buffer_meter" input argument.
#'
...
get_coordinate_inconsistencies <- function(surveys_with_coordinates, 
  ...
  # If no "boundary_buffer_meter" argument is provided, set it to 2000 (by default)
  
if (is.null(boundary_buffer_meter)) {boundary_buffer_meter <- 2000}

  # Assert that all surveys contain columns with coordinates and coordinate errors
  ...
  for (i in seq_along(surveys_with_coordinates)) {
    ...
    df <- get(df_name)
    # Check if both "latitude_dec" and "longitude_dec" are present in the column names
    coords[i] <- "latitude_dec" %in% names(df) && "longitude_dec" %in% names(df)}
  
  assertthat::assert_that(all(coords == TRUE),
                          msg = "One or multiple survey forms given as input do(es) not contain columns 'latitude_dec' and 'longitude_dec'.")
  
  coord_errors <- vector(length = length(surveys_with_coordinates), mode = "logical")
  
  for (i in seq_along(surveys_with_coordinates)) {
    df_name <- surveys_with_coordinates[i]
    df <- get(df_name)
    coord_errors[i] <- "latitude_error" %in% names(df) && "longitude_error" %in% names(df)}
  
  assertthat::assert_that(all(coord_errors == TRUE),
                          msg = "One or multiple survey forms given as input do(es) not contain columns 'latitude_error' and 'longitude_error' with inconsistencies in +/-DDMMSS format.")
...
world_spat <- read_sf("./data/additional_data/partner_boundaries.gpkg", 
                      layer = "world_spat")
german_partners_spat <- read_sf("./data/additional_data/partner_boundaries.gpkg", 
                                layer = "german_partners_spat")
eberswalde_spat <- read_sf("./data/additional_data/partner_boundaries.gpkg", 
                           layer = "eberswalde_spat")
flanders_spat <- read_sf("./data/additional_data/partner_boundaries.gpkg", 
                         layer = "flanders_spat")
wallonia_spat <- read_sf("./data/additional_data/partner_boundaries.gpkg", 
                         layer = "wallonia_spat")
canary_islands_spat <- read_sf("./data/additional_data/partner_boundaries.gpkg", 
                               layer = "canary_islands_spat")
azores_spat <- read_sf("./data/additional_data/partner_boundaries.gpkg", 
                       layer = "azores_spat")
...
for (j in 1:nrow(d_partner_spat))
...
d_partner_spat$spatial_feature_name[j] <- world_spat$NAME_0[ind]} else
  
if (d_partner_spat$code[j] == 1301)  
{d_partner_spat$spatial_feature_source[j] <- "world_spat$NAME_0";
d_partner_spat$spatial_feature_name[j] <- "Sweden"} else
  
if (d_partner_spat$code[j] == 3404)  
{d_partner_spat$spatial_feature_source[j] <- "german_partners_spat$StateName1";
d_partner_spat$spatial_feature_name[j] <- "Schleswig-Holstein"} else
  
if (d_partner_spat$code[j] == 3604)  
{d_partner_spat$spatial_feature_source[j] <- "german_partners_spat$StateName1";
d_partner_spat$spatial_feature_name[j] <- "Sachsen"} else
  
if (d_partner_spat$code[j] == 3704)  
{d_partner_spat$spatial_feature_source[j] <- "german_partners_spat$StateName1";
d_partner_spat$spatial_feature_name[j] <- "Thüringen"} else
  
if (d_partner_spat$code[j] == 2704)  
{d_partner_spat$spatial_feature_source[j] <- "eberswalde_spat"} else
  
if (d_partner_spat$code[j] == 3004)  # several states so just Germany?
{d_partner_spat$spatial_feature_source[j] <- "world_spat$NAME_0";
d_partner_spat$spatial_feature_name[j] <- "Germany"} else
  
if (d_partner_spat$code[j] == 3104)  
{d_partner_spat$spatial_feature_source[j] <- "german_partners_spat$StateName1";
d_partner_spat$spatial_feature_name[j] <- "Mecklenburg-Vorpommern"} else
  
if (d_partner_spat$code[j] == 3204)  
{d_partner_spat$spatial_feature_source[j] <- "german_partners_spat$StateName1";
d_partner_spat$spatial_feature_name[j] <- "Nordrhein-Westfalen"} else
  
if (d_partner_spat$code[j] == 3304)  
{d_partner_spat$spatial_feature_source[j] <- "german_partners_spat$StateName1";
d_partner_spat$spatial_feature_name[j] <- "Rheinland-Pfalz"} else
  
if (d_partner_spat$code[j] == 3504)  
{d_partner_spat$spatial_feature_source[j] <- "german_partners_spat$StateName1";
d_partner_spat$spatial_feature_name[j] <- "Saarland"} else
  
if (d_partner_spat$code[j] == 9804)  
{d_partner_spat$spatial_feature_source[j] <- "german_partners_spat$StateName1";
d_partner_spat$spatial_feature_name[j] <- "Hamburg"} else
  
if (d_partner_spat$code[j] == 2804)  
{d_partner_spat$spatial_feature_source[j] <- "german_partners_spat$StateName1";
d_partner_spat$spatial_feature_name[j] <- "Baden-Württemberg"} else
  
if (d_partner_spat$code[j] == 2904)  
{d_partner_spat$spatial_feature_source[j] <- "german_partners_spat$StateName1";
d_partner_spat$spatial_feature_name[j] <- "Bayern"} else
...
{d_partner_spat$spatial_feature_source[j] <- "flanders_spat"} else
  
if (d_partner_spat$code[j] == 202)  
{d_partner_spat$spatial_feature_source[j] <- "wallonia_spat"} else
  
if (d_partner_spat$code[j] == 6)  
{d_partner_spat$spatial_feature_source[j] <- "world_spat$NAME_0";
d_partner_spat$spatial_feature_name[j] <- "United Kingdom"} else
  
if (d_partner_spat$code[j] == 54)  
{d_partner_spat$spatial_feature_source[j] <- "world_spat$NAME_0";
d_partner_spat$spatial_feature_name[j] <- "Slovakia"} else
  
if (d_partner_spat$code[j] == 58)  
{d_partner_spat$spatial_feature_source[j] <- "world_spat$NAME_0";
d_partner_spat$spatial_feature_name[j] <- "Czech Republic"} else
  
if (d_partner_spat$code[j] == 95)  
{d_partner_spat$spatial_feature_source[j] <- "canary_islands_spat"} else
  
if (d_partner_spat$code[j] == 96)  
{d_partner_spat$spatial_feature_source[j] <- "azores_spat"} else
...
list_coordinate_inconsistencies <- data.frame(survey_form = NULL, 
                                         partner = NULL, 
                                         partner_code = NULL, 
                                         country = NULL,
                                         code_country = NULL, 
...
for (i in 1:length(surveys_with_coordinates))
...
# FSCC_4: Are minutes and seconds in 0-59 range? "Error: out of range (0 - 59)"
  ...
  if (identical(df$survey_year,NULL)) {survey_year_vec <- rep(NA, length(vec))} else
  {survey_year_vec <- df$survey_year[vec]};
  
  if ("profile_pit_id" %in% names(df)) {
    repetition_profile_pit_id_vec <- df$profile_pit_id[vec]} else
  ...
  inconsistency_type <- inconsistency_catalogue$inconsistency_type[
    ...
    list_coordinate_inconsistencies <- rbind(list_coordinate_inconsistencies,
                                     data.frame(survey_form = rep(as.factor(surveys_with_coordinates[i]),length(vec)), 
...
# FSCC_13: Are plot coordinates within the respective partner boundaries?
  ...
  for (j in 1:length(unique(df$partner_code))) { # per partner
  ...
  {names(indiv_partner_spat)[which(names(indiv_partner_spat) == 
                                     spatial_feature_source[2])] <- "partner_name";
  suppressWarnings(indiv_partner_spat <- indiv_partner_spat %>% 
                     filter(partner_name == spatial_feature_name) %>%
                     # to exclude some oversea areas (e.g. for France)
  ...
  # Store inconsistencies in case any coordinates outside the partner boundaries
  # were identified
  ...
  if (identical(df_indiv_partner$survey_year,NULL)) {
    survey_year_k <- rep(NA,length(k))} else
  {survey_year_k <- df_indiv_partner$survey_year[k]};
  if ("profile_pit_id" %in% names(df)) {
    repetition_profile_pit_id_k <- df$profile_pit_id[k]} else
  ...
  list_coordinate_inconsistencies <- rbind(list_coordinate_inconsistencies,
                                    data.frame(survey_form = rep(as.factor(surveys_with_coordinates[i]),length(k)), 
...
assign("list_coordinate_inconsistencies", list_coordinate_inconsistencies, 
       envir=globalenv())
