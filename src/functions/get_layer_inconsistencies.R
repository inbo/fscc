...
#' 
#' @param survey_form Character string - Name of the survey form (lower case and separated by '_') to be evaluated
#' @param solve Logical - Indicates whether obvious mistakes can be solved in this function (if TRUE)
#' @details 
#' This function uses:
...
#'    * layer number (from top to down) for each layer in the soil profile repetition
...
#' FSCC_9:  Do the layer limits equal 0 for multiple depth layers in the given profile?
#' FSCC_11: Does the superior layer limit equal the inferior layer limit?
#' FSCC_7:  Does none of the depth layers of the given profile have known layer limits?
#' FSCC_10: Is there a gap or overlap between adjacent layers?
#'
#' How are the survey forms exactly updated in this function (if "solve" == TRUE)?
#' 1. "som" survey forms: Update "layer_limit_superior" and "layer_limit_inferior":
...
#'        equal 0 for multiple depth layers within a profile (see PIR for rule_ID = "FSCC_9")
...
#'        inferior layer limits based on the column "description" in "d_depth_level_soil")
...
#'    ranking happens based on the superior/upper and inferior/lower layer limits.
...
#'      floor combinations), is shown in the tables "layer_number_two_forest_floor_layers"
#'      and "layer_number_three_forest_floor_layers" in this function.
...
#'      of the layer limits.
#'
#' WARNING - This function may not be optimally efficient and may ideally require 
...
get_layer_inconsistencies <- function(survey_form, solve = NULL) {
  ...
  # For survey forms: "s1_som" and "so_som" and "s1_pfh" and "so_pfh"
  
  # This is a function that evaluates the consistency of depth layers in soil monitoring data
  # (e.g. are there any gaps between adjacent layers in the soil profile?)
  # The function takes two arguments: "survey_form" (the name of the survey_form data to process) 
  # and "solve". The argument "solve" indicates whether obvious mistakes can be 
  # solved in this function (solve == TRUE).
  ...
  #          layers [see PIR for rule_ID = "FSCC_2"] with a factor *(-1)
  #        > Empty layer limits which are clearly a mistake: where layer limits
  #          equal 0 for multiple depth layers within a profile
  #          [see PIR for rule_ID = "FSCC_9"]
  #        > Empty layer limits which are clearly a mistake: where superior layer
  #          limit equals inferior layer limit [see PIR for rule_ID = "FSCC_11"]
  #    -	Gap-fill empty layer_limit_superiors and layer_limit_inferiors
  #        > Retrieve the layer limits from the corresponding "pfh" survey forms
...
if (unlist(strsplit(survey_form, "_"))[2] == "som") {
  ...
  # Retrieve the survey_form data and the "pfh" data (to estimate depth layer limits)
  # and to cross-check forest floor layers in all survey_forms (FSCC_48)
  
df <- get(survey_form, envir = .GlobalEnv)    
pfh <- get(paste0(unlist(strsplit(survey_form, "_"))[1], "_pfh"), envir = .GlobalEnv)
...
progress_bar <- txtProgressBar(min = 0, 
  ...
  # To determine the "layer_number" for forest floor layers without layer limits:
    # The two dataframes below 
    # (layer_number_two_forest_floor_layers and layer_number_three_forest_floor_layers)
    # were made by checking the following for each "unique_repetition" in "so_som":
        # - Which forest floor layers are reported?
        # - What must have been the sequence (layer_number) of the layers?
    # In case it was not clear which layer must have been on top from "code_layer",
    # this information was derived from the total soil organic carbon content
    # in the assumption that: the lower the layer, the higher the % soil organic carbon

layer_number_two_forest_floor_layers <- data.frame(layer_number = c(1, 2),
...
layer_number_three_forest_floor_layers <- c("OL", "OF", "OH")
  ...
  # This dataframe refers to "d_depth_level_soil" in the adds/dictionaries folder,
    # to which columns with the theoretical layer limits
    # ("layer_limit_superior" and "layer_limit_inferior") were added for each code_layer

d_depth_level_soil <- read.csv("./data/additional_data/d_depth_level_soil.csv", 
                               sep=";")

  
  # Initialize variables to store layer numbers and limits
...
df$layer_number <- NA # final layer numbers
                      # if there are overlapping layers between 0 and 10 cm depth
                      # (i.e. M05 & M51 & M01),
                      # layer numbers are not given for the "redundant layer(s)"
                      # (in this case: least detailed redundant layer(s) (i.e. M01) 
df$layer_number_combined_layers <- NA # if there are overlapping layers (e.g. M05 & M51 & M01),
                                      # layer numbers of the least detailed profile (i.e. with M01) 
...
df$layer_limit_superior_orig <- df$layer_limit_superior}
  ...
  # to store inconsistencies found in the survey data:
...
for (i in 1:length(unique(df$unique_survey_repetition)))
  
{# Determine index of layers with the given unique_survey_repetition in df
vec <- which(unique(df$unique_survey_repetition)[i]==df$unique_survey_repetition); 
...
# Determine which unique_survey_repetitions exist for the given plot_ID in "so_som"
...
survey_repetitions_som_other <- 
  unique(som_other$unique_survey_repetition[which(som_other$plot_ID == plot_ID_i)])
survey_repetitions_pfh_other <- 
  unique(pfh_other$unique_survey_profile[which(pfh_other$plot_ID == plot_ID_i)])
...
# Create a data frame with metadata of the repetitions that exist for the given plot_ID
...
df_ff_i <- df[vec_som[which(!duplicated(df$unique_survey_repetition[vec_som]))],
   which(names(df) %in% c("partner_code", "survey_year", "code_plot", "plot_ID", 
                          "repetition", "unique_survey_repetition"))]
names(df_ff_i)[which(names(df_ff_i) == "repetition")] <- "repetition_profile_pit_id"


pfh_ff_i <- pfh[vec_pfh[which(!duplicated(pfh$unique_survey_profile[vec_pfh]))],
    which(names(pfh) %in% c("partner_code", "survey_year", "code_plot", "plot_ID", 
                            "profile_pit_id", "unique_survey_profile"))]
names(pfh_ff_i)[which(names(pfh_ff_i) == "profile_pit_id")] <- "repetition_profile_pit_id"
names(pfh_ff_i)[which(names(pfh_ff_i) == "unique_survey_profile")] <- "unique_survey_repetition"


som_other_ff_i <- som_other[vec_som_other[
  which(!duplicated(som_other$unique_survey_repetition[vec_som_other]))],
          which(names(som_other) %in% c("partner_code", "survey_year", "code_plot", "plot_ID", 
                                        "repetition", "unique_survey_repetition"))]
names(som_other_ff_i)[which(names(som_other_ff_i) == "repetition")] <- 
...
pfh_other_ff_i <- pfh_other[vec_pfh_other[
  which(!duplicated(pfh_other$unique_survey_profile[vec_pfh_other]))],
          which(names(pfh_other) %in% c("partner_code", "survey_year", "code_plot", "plot_ID", 
                                        "profile_pit_id", "unique_survey_profile"))]
names(pfh_other_ff_i)[which(names(pfh_other_ff_i) == "profile_pit_id")] <- 
  "repetition_profile_pit_id"
names(pfh_other_ff_i)[which(names(pfh_other_ff_i) == "unique_survey_profile")] <- 
  "unique_survey_repetition"


# Merge these four dataframes to one data_frame "list_ff_meta_i"

list_ff_meta_i <- cbind(data.frame("survey_form" =
                        ...
                        data.frame("number_ff_layers" = rep(NA, (length(survey_repetitions_som) +
...
for (j in 1:(length(survey_repetitions_som) + length(survey_repetitions_pfh) +
             length(survey_repetitions_som_other) + length(survey_repetitions_pfh_other))) {
  ...
  if (j <= length(survey_repetitions_som)) {
    ...
    forest_floor_layers_som <- append(forest_floor_layers_som,
                                      list(forest_floor_j))
    names(forest_floor_layers_som)[j] <- as.character(survey_repetitions_som[j])
  ...
  if ((length(survey_repetitions_pfh) > 0) & 
    ...
    vec_pfh_j <- which(pfh$unique_survey_profile == survey_repetitions_pfh[j_pfh])
    ...
    forest_floor_layers_pfh <- append(forest_floor_layers_pfh,
                                      list(forest_floor_j))
    names(forest_floor_layers_pfh)[j_pfh] <- as.character(survey_repetitions_pfh[j_pfh])
    ...
    if ((length(survey_repetitions_som_other) > 0) & 
      ...
      j_som_other <- (j - (length(survey_repetitions_som) + length(survey_repetitions_pfh)))
      ...
      forest_floor_j <- as.character(som_other$code_layer[vec_som_other_j[
                  which(som_other$layer_type[vec_som_other_j] == "forest_floor")]])
      list_ff_meta_i$number_ff_layers[j] <- length(forest_floor_j)
    ...
    if ((length(survey_repetitions_pfh_other) > 0) & 
      ...
      j_pfh_other <- (j - (length(survey_repetitions_som) + length(survey_repetitions_pfh) + 
                             length(survey_repetitions_som_other)))
      ...
      forest_floor_j <- as.character(pfh_other$horizon_master[vec_pfh_other_j[
                  which(pfh_other$layer_type[vec_pfh_other_j] == "forest_floor")]])
      list_ff_meta_i$number_ff_layers[j] <- length(forest_floor_j)
...
# sometimes absent in the given plot_ID
 ...
 if ( # If at least 1 survey repetition does not have forest floor layers
...
# FSCC_48: Caution: for the given plot_ID, forest floor layers over different 
   ...
   # not consistently reported: forest floor layers are not reported (i.e. absent) 
   # for repetition/profile_pit_id = xxx
   
   # Retrieve information about the inconsistency from "inconsistency_catalogue"
   ...
   caution_ff <- paste0(caution_ff, ": forest floor layers are absent for ")

   # Add a column to "list_ff_meta_i" with a unique value per survey_year per survey_form
   ...
   # Create a vector "vec_ff_absent_inconsistent" which tells, using the "survey_form_year"
   # column, whether there are repetitions with forest floor layers + without
   ...
   vec_ff_present_inconsistent <- which(((list_ff_meta_i$survey_form_year == 
   ...
   vec_ff_inconsistent <- vec_ff_absent[which(vec_ff_absent_inconsistent==TRUE)]
   surveys_inconsistent <- unique(list_ff_meta_i$survey_form_year[vec_ff_inconsistent])
   
   # Determine indices in "list_ff_meta_i" of unique_survey_repetitions without
   # forest floor layers for which all other unique_survey_repetitions within
   # the same survey year and survey form also DO NOT contain forest floor layers
   
   vec_ff_consistent <- vec_ff_absent[which(is.na(vec_ff_absent_inconsistent))]
   surveys_consistent <- unique(list_ff_meta_i$survey_form_year[vec_ff_consistent])
   ...
   for (j in seq_along(surveys_inconsistent)) {
     vec_ff_inconsistent_j <- 
       vec_ff_inconsistent[which(list_ff_meta_i$survey_form_year[vec_ff_inconsistent] == 
                                   surveys_inconsistent[j])]
     
     vec_ff_inconsistent_j_character <- as_character_summary(vec_ff_inconsistent_j)
     ...
     if (j < length(surveys_inconsistent)) {caution_ff <- paste0(caution_ff,", ")}
     if (j == length(surveys_inconsistent)-1) {caution_ff <- paste0(caution_ff,"and ")}}
   ...
   for (j in seq_along((surveys_consistent))) {
     ...
     if (j == 1) {caution_ff <- paste0(caution_ff, "survey year ")}
     
     caution_ff <- paste0(caution_ff,
                           survey_year_j, " (", survey_form_j, ")")
     if (j < length(surveys_consistent) -1) {caution_ff <- paste0(caution_ff,", ")}
     if (j == length(surveys_consistent)-1) {caution_ff <- paste0(caution_ff," and ")}}
   ...
   caution_ff <- paste0(caution_ff,
     ". Meanwhile, forest floor layers have been reported for the records listed in PIR, i.e. in profiles from survey year ")
   
   if (length(unique(list_ff_meta_i$survey_form[vec_ff_present])) == 1) {
     caution_ff <- paste0(caution_ff,
                           as_character_summary(unique(list_ff_meta_i$survey_year[vec_ff_present])),
                           " (", 
   ...
   # Create a vector with row indices of forest floor layers (to add to inconsistency list)
   
   vec_ff_som <- vec_som[which(df$layer_type[vec_som] == "forest_floor")]
   vec_ff_pfh <- vec_pfh[which(pfh$layer_type[vec_pfh] == "forest_floor")]
   vec_ff_som_other <- vec_som_other[which(som_other$layer_type[vec_som_other] == "forest_floor")]
   vec_ff_pfh_other <- vec_pfh_other[which(pfh_other$layer_type[vec_pfh_other] == "forest_floor")]
   ...
   if ("code_layer_original" %in% names(som_other)) {
     vec_code_layer <- as.character(som_other$code_layer_original[vec_ff_som_other])} else
     {vec_code_layer <- as.character(som_other$code_layer[vec_ff_som_other])}
   
   # "so_som"
   
   if (!identical(vec_ff_som, integer(0))) {
   list_layer_inconsistencies <- rbind(list_layer_inconsistencies, 
                                       data.frame(survey_form = (rep(survey_form,length(vec_ff_som))), 
                                                  ...
                                                  rule_ID = rule_ID,
                                                  non_duplicated_error_type_per_record = rep(FALSE,length(vec_ff_som)),
   ...
   list_layer_inconsistencies <- rbind(list_layer_inconsistencies, 
                                       data.frame(survey_form = (rep(survey_2,length(vec_ff_pfh))), 
                                                  ...
                                                  inconsistency_type = inconsistency_type,
                                                  rule_ID = rule_ID,
                                                  non_duplicated_error_type_per_record = rep(FALSE,length(vec_ff_pfh)),
   ...
   list_layer_inconsistencies <- rbind(list_layer_inconsistencies, 
                                       data.frame(survey_form = (rep(survey_3,length(vec_ff_som_other))), 
                                                  ...
                                                  repetition_profile_pit_id = som_other$repetition[vec_ff_som_other],
                                                  ...
                                                  inconsistency_type = inconsistency_type,
                                                  rule_ID = rule_ID,
                                                  non_duplicated_error_type_per_record = rep(FALSE,length(vec_ff_som_other)),
                                                  change_date = som_other$change_date[vec_ff_som_other],
                                                  download_date = rep(download_date_pir,length(vec_ff_som_other))))}
   ...
   list_layer_inconsistencies <- rbind(list_layer_inconsistencies, 
                                       data.frame(survey_form = (rep(survey_4,length(vec_ff_pfh_other))), 
                                                  ...
                                                  code_layer_horizon_master = pfh_other$horizon_master[vec_ff_pfh_other],
                                                  repetition_profile_pit_id = pfh_other$profile_pit_id[vec_ff_pfh_other],
                                                  ...
                                                  parameter_value = as.character(pfh_other$horizon_master[vec_ff_pfh_other]),
                                                  inconsistency_reason = inconsistency_reason,
                                                  inconsistency_type = inconsistency_type,
                                                  rule_ID = rule_ID,
                                                  non_duplicated_error_type_per_record = rep(FALSE,length(vec_ff_pfh_other)),
                                                  change_date = pfh_other$change_date[vec_ff_pfh_other],
                                                  download_date = rep(download_date_pir,length(vec_ff_pfh_other))))}
   
   # Change "non_duplicated_error_type_per_record" so that there is only one
   # unique inconsistency per plot_ID
   
   length_inconsistency <- length(c(vec_ff_som, vec_ff_pfh, vec_ff_som_other, vec_ff_pfh_other))
...
if (any(!is.na(df$layer_limit_superior_orig[vec_ff])) &
  ...
  # Only for profiles without peat since peat soils usually have a special sequence
  
if (all(df$layer_limit_inferior_orig[vec_ff_sup] >= 0) &
    all(df$layer_limit_superior_orig[vec_ff_inf] >= 0) &
    any(c(df$layer_limit_inferior_orig[vec_ff_sup], df$layer_limit_superior_orig[vec_ff_inf]) > 0) &
    length(vec_ff) >= 1 &
  ...
  if ("code_layer_original" %in% names(df)) {
    vec_code_layer <- as.character(df$code_layer_original[vec_ff_sup])} else
  ...
  list_layer_inconsistencies <- rbind(list_layer_inconsistencies, 
                                      data.frame(survey_form = as.factor(rep(survey_form,length(vec_ff_sup))), 
                                                 ...
                                                 inconsistency_type = inconsistency_type,
                                                 rule_ID = rule_ID,
                                                 non_duplicated_error_type_per_record = rep(TRUE,length(vec_ff_sup)),
  ...
  if ("code_layer_original" %in% names(df)) {
    vec_code_layer <- as.character(df$code_layer_original[vec_ff_inf])} else
  ...
  list_layer_inconsistencies <- rbind(list_layer_inconsistencies, 
                                      data.frame(survey_form = as.factor(rep(survey_form,length(vec_ff_inf))), 
                                                 ...
                                                 inconsistency_type = inconsistency_type,
                                                 rule_ID = rule_ID,
                                                 non_duplicated_error_type_per_record = rep(FALSE,length(vec_ff_inf)),
  ...
  # with *(-1) so that they become negative
   ...
   if (solve == TRUE) {
    df$layer_limit_inferior[vec_ff_inf] <- (-1)*(df$layer_limit_inferior_orig[vec_ff_inf]);
    df$layer_limit_superior[vec_ff_sup] <- (-1)*(df$layer_limit_superior_orig[vec_ff_sup])}}}
...
if (all(df$layer_limit_inferior_orig[vec_bg_inf] <= 0) &
  ...
  if ("code_layer_original" %in% names(df)) {
    vec_code_layer <- as.character(df$code_layer_original[vec_bg_sup])} else
  ...
  list_layer_inconsistencies <- rbind(list_layer_inconsistencies, 
                                      data.frame(survey_form = as.factor(rep(survey_form,length(vec_bg_sup))), 
                                                 ...
                                                 inconsistency_type = inconsistency_type,
                                                 rule_ID = rule_ID,
                                                 non_duplicated_error_type_per_record = rep(TRUE,length(vec_bg_sup)),
  ...
  if ("code_layer_original" %in% names(df)) {
    vec_code_layer <- as.character(df$code_layer_original[vec_bg_inf])} else
  ...
  list_layer_inconsistencies <- rbind(list_layer_inconsistencies, 
                                      data.frame(survey_form = as.factor(rep(survey_form,length(vec_bg_inf))), 
                                                 ...
                                                 inconsistency_type = inconsistency_type,
                                                 rule_ID = rule_ID,
                                                 non_duplicated_error_type_per_record = rep(FALSE,length(vec_bg_inf)),
  ...
  if (solve == TRUE) {
    df$layer_limit_inferior[vec_bg_inf] <- (-1)*(df$layer_limit_inferior_orig[vec_bg_inf]);
    df$layer_limit_superior[vec_bg_sup] <- (-1)*(df$layer_limit_superior_orig[vec_bg_sup])}}}
...
for (j in vec) {
  ...
  if ("code_layer_original" %in% names(df)) {
    vec_code_layer <- as.character(df$code_layer_original[j])} else
...
# FSCC_40: the reported "code_layer" is unknown
      ...
      list_layer_inconsistencies <- rbind(list_layer_inconsistencies, 
                                          data.frame(survey_form = as.factor(rep(survey_form,length(j))), 
                                                     ...
                                                     download_date = rep(download_date_pir,length(j))))} else
...
# FSCC_39: reported layer limits do not correspond with code_layer
  ...
  {limit_superior_theoretical <- d_depth_level_soil$layer_limit_superior[ind];
   ...
   # Store information about the inconsistency in "list_layer_inconsistencies"
     ...
     inconsistency_reason <- inconsistency_catalogue$inconsistency_reason[
       which(inconsistency_catalogue$rule_ID == rule_ID)];
     inconsistency_reason <- paste0(inconsistency_reason, " (i.e. ", 
                                    limit_superior_theoretical, " cm).")
     inconsistency_type <- inconsistency_catalogue$inconsistency_type[
       which(inconsistency_catalogue$rule_ID == rule_ID)];
     ...
     list_layer_inconsistencies <- rbind(list_layer_inconsistencies, 
                                         data.frame(survey_form = as.factor(rep(survey_form,length(j))), 
                                                    ...
                                                    parameter_value = as.character(df$layer_limit_superior_orig[j]),
                                                    inconsistency_reason = inconsistency_reason,
                                                    inconsistency_type = inconsistency_type,
                                                    rule_ID = rule_ID,
                                                    non_duplicated_error_type_per_record = rep(TRUE,length(j)),
   ...
   if (is.na(df$layer_limit_inferior_orig[j]) |
     ...
     list_layer_inconsistencies <- rbind(list_layer_inconsistencies, 
                                         data.frame(survey_form = as.factor(rep(survey_form,length(j))), 
                                                    ...
                                                    parameter_unit = (rep("cm",length(j))),
                                                    parameter_value = as.character(df$layer_limit_inferior_orig[j]),
                                                    inconsistency_reason = inconsistency_reason,
                                                    inconsistency_type = inconsistency_type,
                                                    rule_ID = rule_ID,
                                                    non_duplicated_error_type_per_record = rep(TRUE,length(j)),
...
if ((any(!is.na(df$layer_limit_superior_orig[vec]))) &
  ...
  if ("code_layer_original" %in% names(df)) {
    vec_code_layer <- as.character(df$code_layer_original[vec_inconsistency])} else
    {vec_code_layer <- as.character(df$code_layer[vec_inconsistency])}
  
  list_layer_inconsistencies <- rbind(list_layer_inconsistencies, 
                                      data.frame(survey_form = as.factor(rep(survey_form,length(vec_inconsistency))), 
                                                 ...
                                                 parameter = as.factor(rep("layer_limit_superior",length(vec_inconsistency))), 
                                                 parameter_unit = as.factor(rep("cm",length(vec_inconsistency))),
                                                 parameter_value = df$layer_limit_superior_orig[vec_inconsistency],
                                                 inconsistency_reason = inconsistency_reason,
                                                 inconsistency_type = inconsistency_type,
                                                 rule_ID = rule_ID,
                                                 non_duplicated_error_type_per_record = rep(TRUE,length(vec_inconsistency)),
                                                 change_date = df$change_date[vec_inconsistency],
                                                 download_date = rep(download_date_pir,length(vec_inconsistency))))};
...
if ((any(!is.na(df$layer_limit_inferior_orig[vec]))) &
  ...
  list_layer_inconsistencies <- rbind(list_layer_inconsistencies, 
                                      data.frame(survey_form = as.factor(rep(survey_form,length(vec_inconsistency))), 
                                                 ...
                                                 repetition_profile_pit_id = df$repetition[vec_inconsistency],
                                                 code_line = df$code_line[vec_inconsistency],
                                                 parameter = as.factor(rep("layer_limit_inferior",length(vec_inconsistency))), 
                                                 parameter_unit = as.factor(rep("cm",length(vec_inconsistency))),
                                                 parameter_value = df$layer_limit_inferior_orig[vec_inconsistency],
                                                 inconsistency_reason = inconsistency_reason,
                                                 inconsistency_type = inconsistency_type,
                                                 rule_ID = rule_ID,
                                                 non_duplicated_error_type_per_record = rep(TRUE,length(vec_inconsistency)),
...
# FSCC_9 Error reason: the layer limits equal 0 for multiple depth layers in the given profile
...
{if ((unique(df$layer_limit_superior_orig[vec[
  ...
  # Store information about the inconsistency in "list_layer_inconsistencies"
        ...
        if ("code_layer_original" %in% names(df)) {
          vec_code_layer <- as.character(df$code_layer_original[vec_error])} else
          {vec_code_layer <- as.character(df$code_layer[vec_error])}
        
        list_layer_inconsistencies <- rbind(list_layer_inconsistencies, 
                                    data.frame(survey_form = as.factor(rep(survey_form,length(vec_error))), 
                                               ...
                                               parameter = as.factor(rep("layer_limit_superior",length(vec_error))), 
                                               parameter_unit = as.factor(rep("cm",length(vec_error))),
                                               parameter_value = df$layer_limit_superior_orig[vec_error],
                                               inconsistency_reason = inconsistency_reason,
                                               inconsistency_type = inconsistency_type,
                                               rule_ID = rule_ID,
                                               non_duplicated_error_type_per_record = rep(TRUE,length(vec_error)),
  ...
  # Store information about the inconsistency in "list_layer_inconsistencies"
      ...
      list_layer_inconsistencies <- rbind(list_layer_inconsistencies, 
                                   data.frame(survey_form = as.factor(rep(survey_form,length(vec_error))), 
                                              ...
                                              repetition_profile_pit_id = df$repetition[vec_error],
                                              code_line = df$code_line[vec_error],
                                              parameter = as.factor(rep("layer_limit_inferior",length(vec_error))), 
                                              parameter_unit = as.factor(rep("cm",length(vec_error))),
...
{if (!is.na(df$layer_limit_superior_orig[j]) & !is.na(df$layer_limit_inferior_orig[j]))
...
### Step 2: Try to gap-fill any empty layer_limit_superior and layer_limit_inferior #----

  # If there is at least one layer without limit information

if ((any(is.na(df$layer_limit_inferior[vec])) | any(is.na(df$layer_limit_superior[vec]))))
  
  # Get the index of empty cells in layer_limit_superior and layer_limit_inferior
  
{vec_empty_layer_limit_superior <- vec[which(is.na(df$layer_limit_superior[vec]))];
vec_empty_layer_limit_inferior <- vec[which(is.na(df$layer_limit_inferior[vec]))];

# Option 2.1: Can we fill these layers based on pfh? (mainly for organic layers)
...
{for (j in vec_empty_layer_limit_superior)
  ...
  if ("code_layer_original" %in% names(df)) {
    ...
    {df$layer_limit_superior[j] <- 
      pfh$horizon_limit_up[vec_pfh[match(vec_code_layer,
                                         pfh$horizon_master[vec_pfh])]]} else
          ...
          if (vec_code_layer == "OLF" &
              ...
              {df$layer_limit_superior[j] <- 
                pfh$horizon_limit_up[vec_pfh[match("OL",
                                                   pfh$horizon_master[vec_pfh])]]} else
          ...
          if (vec_code_layer == "OFH" &
              ...
              {df$layer_limit_superior[j] <- 
                pfh$horizon_limit_up[vec_pfh[match("OF",
    ...
    {df$layer_limit_inferior[j] <- 
      pfh$horizon_limit_low[vec_pfh[match(vec_code_layer,
          ...
          if (vec_code_layer == "OLF" &
              ...
              !is.na(match("OF",pfh$horizon_master[vec_pfh])))
              {df$layer_limit_inferior[j] <- 
               pfh$horizon_limit_low[vec_pfh[match("OF",
                                                   pfh$horizon_master[vec_pfh])]]} else
          if (vec_code_layer == "OFH" &
              !is.na(match("OF",pfh$horizon_master[vec_pfh])) &
              !is.na(match("OH",pfh$horizon_master[vec_pfh])))
              {df$layer_limit_inferior[j] <- 
                pfh$horizon_limit_low[vec_pfh[match("OH",
                                                    pfh$horizon_master[vec_pfh])]]}         
  }}
};
...
{if (df$layer_limit_inferior_orig[vec[which(df$code_layer[vec] == "OF")]] == 3)
...
if ("code_layer_original" %in% names(df)) {
  vec_code_layer <- as.character(df$code_layer_original[ind_error])} else
  ...
  list_layer_inconsistencies <- rbind(list_layer_inconsistencies, 
                              data.frame(survey_form = as.factor(rep(survey_form,length(ind_error))), 
                                         ...
                                         parameter = as.factor(rep("layer_limit_inferior",length(ind_error))), 
                                         parameter_unit = as.factor(rep("cm",length(ind_error))),
                                         parameter_value = df$layer_limit_inferior_orig[ind_error],
                                         inconsistency_reason = inconsistency_reason,
                                         inconsistency_type = inconsistency_type,
                                         rule_ID = rule_ID,
                                         non_duplicated_error_type_per_record = rep(TRUE,length(ind_error)),
  ...
  if (solve == TRUE) {df$layer_limit_inferior[ind_error] <- (-3)}
}}
...
if ("code_layer_original" %in% names(df)) {
  vec_code_layer <- as.character(df$code_layer_original[vec])} else
  ...
  list_layer_inconsistencies <- rbind(list_layer_inconsistencies, 
                               data.frame(survey_form = as.factor(rep(survey_form,length(vec))), 
                                          ...
                                          download_date = rep(download_date_pir,length(vec))))} else
...
### Step 3: The layers are ordered (layer_number) from top to bottom
  ...
  if (any(!is.na(df$layer_limit_inferior[vec]) & !is.na(df$layer_limit_superior[vec])))
  ...
  {# if forest floor has no layer limit information
    ...
    {# if there is one forest floor layer
     ...
     # use the data frame "layer_number_two_forest_floor_layers"
        ...
        if (!is.na(match(layers[1],layer_number_two_forest_floor_layers[,5])) &
            !is.na(match(layers[2],layer_number_two_forest_floor_layers[,5])))
          
        # if it is in Bulgaria  
        {if (df$code_country[vec[1]] == 63) 
        {if (df$code_layer[vec_ff[1]] == layer_number_two_forest_floor_layers[1,6])
                {df$layer_number[vec_ff[1]] <- 1;
                df$layer_number[vec_ff[2]] <- 2} else {
                df$layer_number[vec_ff[1]] <- 2;
                df$layer_number[vec_ff[2]] <- 1}} else {
                  
        # if it is in France
         if (df$code_layer[vec_ff[1]] == layer_number_two_forest_floor_layers[1,5])
                {df$layer_number[vec_ff[1]] <- 1;
                df$layer_number[vec_ff[2]] <- 2} else {
                df$layer_number[vec_ff[1]] <- 2;
                df$layer_number[vec_ff[2]] <- 1}}} else {
        ...
        # if the forest floor layers are not called "O1" and "O2"
         ...
         for (j in c(3:4,7:11))
                {if (!is.na(match(layers[1],layer_number_two_forest_floor_layers[,j])) &
                     !is.na(match(layers[2],layer_number_two_forest_floor_layers[,j])))
                {if (df$code_layer[vec_ff[1]] == layer_number_two_forest_floor_layers[1,j])
                    {df$layer_number[vec_ff[1]] <- 1;
                    df$layer_number[vec_ff[2]] <- 2} else {
                    df$layer_number[vec_ff[1]] <- 2;
                    df$layer_number[vec_ff[2]] <- 1}}}}} else
    ...
    # use the data frame "layer_number_three_forest_floor_layers"
        ...
        {df$layer_number[vec_ff[match(layer_number_three_forest_floor_layers[1],layers)]] <- 1;
        df$layer_number[vec_ff[match(layer_number_three_forest_floor_layers[2],layers)]] <- 2;
        df$layer_number[vec_ff[match(layer_number_three_forest_floor_layers[3],layers)]] <- 3}}};
   ...
   # Check which layers have layer limit information but no layer number yet
      ...
      if (!identical(which(is.na(df$layer_number[vec]) &
                           ...
                           !is.na(df$layer_limit_inferior[vec])), integer(0)))
        
      {vec_nonempty <- vec[which(is.na(df$layer_number[vec]) &
                                 !is.na(df$layer_limit_superior[vec]) &
                                 !is.na(df$layer_limit_inferior[vec]))];
    
    # Check if any layer_number(s) have already been assigned for forest floor layers
    # in the given profile by means of 'layer_number_max',
    # which equals the maximum existing layer number or 0
      
          if (all(is.na(df$layer_number[vec]))) {layer_number_max <- 0} else
          ...
          #   Because of which it doesn't get a layer number in "df$layer_number"
          # - In "df$layer_number_combined_layers", "M05" and "M51" are considered
          #   redundant, because of which they don't get a layer number in
          #   "df$layer_number_combined_layers"
          
          if (!identical(which(df$code_layer[vec_nonempty] == "M01"), integer(0)) &
              !identical(which(df$code_layer[vec_nonempty] == "M05"), integer(0)) &
              !identical(which(df$code_layer[vec_nonempty] == "M51"), integer(0)))
            
          {df$layer_number[vec_nonempty[which(df$code_layer[vec_nonempty] == "M05")]] <- 
             1 + layer_number_max;
           df$layer_number[vec_nonempty[which(df$code_layer[vec_nonempty] == "M51")]] <- 
             2 + layer_number_max;
           ...
           if (all(rank(as.numeric(df$layer_limit_superior[vec_nonempty_remaining])) ==
               ...
               # Rank the remaining layers based on layer_limit_superior
                ...
                df$layer_number[vec_nonempty_remaining] <- rank(as.numeric(
                  df$layer_limit_superior[vec_nonempty_remaining])) + layer_number_max;
                 
                
                # Also add a ranking to the column "df$layer_number_combined_layers"
                # although this won't be saved at the end of the function
                
                if (all(is.na(df$layer_number_combined_layers[vec]))) 
                                  {layer_number_max_combined_layers <- 0} else
                {layer_number_max_combined_layers <- 
                     as.numeric(max(df$layer_number_combined_layers[vec[
                              which(!is.na(df$layer_number_combined_layers[vec]))]]))}
                
                df$layer_number_combined_layers[vec_nonempty_remaining] <- 
                      rank(as.numeric(df$layer_limit_superior[vec_nonempty_remaining])) +
                      layer_number_max_combined_layers} else
           
            # If there is a difference between ranking layer_limit_superior 
            # vs ranking layer_limit_inferior:
            # Fill the columns "layer_number_superior" and "layer_number_inferior"
            # Do not fill the "layer_number" and "layer_number_combined_layers" columns
...
vec_layer_number <- vec_nonempty[order(df$layer_number[vec_nonempty])];
    ...
    {for (j in vec_layer_number[1:(length(vec_layer_number)-1)])
          ...
          {if (df$layer_limit_inferior[j] !=
               df$layer_limit_superior[vec_layer_number[match(j,vec_layer_number)+1]])
    ...
    # Store information about the inconsistency in "list_layer_inconsistencies"
          ...
          # Mention that this was based on a value "estimated by FSCC"
            ...
            if ((!is.na(df$layer_limit_inferior[j]) & 
                 is.na(df$layer_limit_inferior_orig[j])) |
                (df$layer_limit_inferior[j] != df$layer_limit_inferior_orig[j]))
               {parameter_value_error <- paste0(df$layer_limit_inferior[j], 
                                                " (estimated by FSCC)")} else
          ...
          if ("code_layer_original" %in% names(df)) {
            ...
            if ((!is.na(df$layer_limit_superior[j_next]) & 
                ...
                (df$layer_limit_superior[j_next] != df$layer_limit_superior_orig[j_next]))
            ...
            if ("code_layer_original" %in% names(df)) {
              vec_code_layer <- as.character(df$code_layer_original[j_next])} else
              {vec_code_layer <- as.character(df$code_layer[j_next])}
            
            list_layer_inconsistencies <- rbind(list_layer_inconsistencies, 
                                         data.frame(survey_form = as.factor(rep(survey_form,length(j_next))), 
                                                    ...
                                                    parameter = as.factor(rep("layer_limit_superior",length(j_next))), 
                                                    parameter_unit = as.factor(rep("cm",length(j_next))),
                                                    parameter_value = parameter_value_error,
                                                    inconsistency_reason = inconsistency_reason,
                                                    inconsistency_type = inconsistency_type,
                                                    rule_ID = rule_ID,
                                                    non_duplicated_error_type_per_record = rep(FALSE,length(j_next)),
...
# df$layer_number_superior  
# df$layer_number_inferior  
# df$layer_number_combined_layers

df <- df[,-which(names(df) %in% c("layer_number_superior", "layer_number_inferior",
...
close(progress_bar)}; # end of "som" part
  ...
  if (unlist(strsplit(survey_form, "_"))[2] == "pfh") {  
    ...
    progress_bar <- txtProgressBar(min = 0, max = length(unique(df$unique_survey_profile)), style = 3)
    ...
    # In addition, a list of redundant layers of the given survey form will be created
    # (this is internal information for FSCC, so this is not included in PIR)
    # This "list_redundant_layers" has the following format:
    
    list_redundant_layers <- data.frame(survey_form = NULL, 
                                        partner = NULL, 
                                        partner_code = NULL, 
                                        country = NULL,
                                        code_country = NULL, 
   ...
   # Read the R code of the "get_redundant_layers()" function
    ...
    for (i in 1:length(unique(df$unique_survey_profile)))
      
      # Determine index of layers with the given unique_survey_profile in df
      
    {vec <- which(unique(df$unique_survey_profile)[i] == df$unique_survey_profile);
      ...
      if (all(df$horizon_limit_low_orig[vec_ff_sup] >= 0) &
        ...
        list_layer_inconsistencies <- rbind(list_layer_inconsistencies, 
                                            data.frame(survey_form = as.factor(rep(survey_form,length(vec_ff_sup))), 
                                                       ...
                                                       code_line = df$code_line[vec_ff_sup],
                                                       parameter = as.factor(rep("horizon_limit_up",length(vec_ff_sup))), 
        ...
        list_layer_inconsistencies <- rbind(list_layer_inconsistencies, 
                                            data.frame(survey_form = as.factor(rep(survey_form,length(vec_ff_inf))), 
                                                       ...
                                                       code_line = df$code_line[vec_ff_inf],
                                                       parameter = as.factor(rep("horizon_limit_low",length(vec_ff_inf))), 
        ...
        # with *(-1) so that they become negative
           ...
           if (solve == TRUE) {
          df$horizon_limit_low[vec_ff] <- (-1)*(df$horizon_limit_low_orig[vec_ff]);
          df$horizon_limit_up[vec_ff] <- (-1)*(df$horizon_limit_up_orig[vec_ff])}}};
      ...
      if (all(df$horizon_limit_low_orig[vec_bg_inf] <= 0) &
        ...
        list_layer_inconsistencies <- rbind(list_layer_inconsistencies, 
                                            data.frame(survey_form = as.factor(rep(survey_form,length(vec_bg_sup))), 
                                                       ...
                                                       code_line = df$code_line[vec_bg_sup],
                                                       parameter = as.factor(rep("horizon_limit_up",length(vec_bg_sup))), 
        ...
        list_layer_inconsistencies <- rbind(list_layer_inconsistencies, 
                                            data.frame(survey_form = as.factor(rep(survey_form,length(vec_bg_inf))), 
                                                       ...
                                                       code_line = df$code_line[vec_bg_inf],
                                                       parameter = as.factor(rep("horizon_limit_low",length(vec_bg_inf))), 
        ...
        # with *(-1) so that they become positive
          ...
          if (solve == TRUE) {
          df$horizon_limit_low[vec_bg] <- (-1)*(df$horizon_limit_low_orig[vec_bg]);
          df$horizon_limit_up[vec_bg] <- (-1)*(df$horizon_limit_up_orig[vec_bg])}}};
    ...
    {if (df$horizon_limit_low_orig[vec[which(df$horizon_master[vec] == "OL")]] == 1)
    ...
    list_layer_inconsistencies <- rbind(list_layer_inconsistencies, 
                                        data.frame(survey_form = as.factor(rep(survey_form,length(ind_error))), 
                                                   ...
                                                   code_line = df$code_line[ind_error],
                                                   parameter = as.factor(rep("horizon_limit_low",length(ind_error))), 
                                                   parameter_unit = as.factor(rep("cm",length(ind_error))),
                                                   ...
                                                   change_date = df$change_date[ind_error],
                                                   download_date = rep(download_date_pir,length(ind_error))));    
    if (solve == TRUE) {df$horizon_limit_low[ind_error] <- (-1)}
    }};
    
    # One value in the given profile seems to have the wrong sign
    
    if (unique(df$unique_survey_profile)[i] == "3004_2007_684_684")  
    {if (df$horizon_limit_low_orig[vec[which(df$horizon_master[vec] == "OF")]] == 2.4)
    ...
    list_layer_inconsistencies <- rbind(list_layer_inconsistencies, 
                                        data.frame(survey_form = as.factor(rep(survey_form,length(ind_error))), 
                                                   ...
                                                   change_date = df$change_date[ind_error],
                                                   download_date = rep(download_date_pir,length(ind_error))));    
    if (solve == TRUE) {df$horizon_limit_low[ind_error] <- (-2.4)}
    }};
    ...
    if (any(is.na(df$horizon_limit_up_orig[vec]))) {
      ...
      list_layer_inconsistencies <- rbind(list_layer_inconsistencies, 
                                          data.frame(survey_form = as.factor(rep(survey_form,length(vec_inconsistency))), 
                                                     ...
                                                     code_plot = df$code_plot[vec_inconsistency], 
                                                     plot_ID = df$plot_ID[vec_inconsistency],
                                                     code_layer_horizon_master = df$horizon_master[vec_inconsistency],
                                                     repetition_profile_pit_id = df$profile_pit_id[vec_inconsistency],
                                                     code_line = df$code_line[vec_inconsistency],
                                                     parameter = as.factor(rep("horizon_limit_up",length(vec_inconsistency))), 
                                                     parameter_unit = as.factor(rep("cm",length(vec_inconsistency))),
                                                     parameter_value = df$horizon_limit_up_orig[vec_inconsistency],
                                                     inconsistency_reason = inconsistency_reason,
                                                     inconsistency_type = inconsistency_type,
                                                     rule_ID = rule_ID,
                                                     non_duplicated_error_type_per_record = rep(TRUE,length(vec_inconsistency)),
    ...
    if (any(is.na(df$horizon_limit_low_orig[vec]))) {
      ...
      list_layer_inconsistencies <- rbind(list_layer_inconsistencies, 
                                          data.frame(survey_form = as.factor(rep(survey_form,length(vec_inconsistency))), 
                                                     ...
                                                     repetition_profile_pit_id = df$profile_pit_id[vec_inconsistency],
                                                     code_line = df$code_line[vec_inconsistency],
                                                     parameter = as.factor(rep("horizon_limit_low",length(vec_inconsistency))), 
                                                     parameter_unit = as.factor(rep("cm",length(vec_inconsistency))),
                                                     parameter_value = df$horizon_limit_low_orig[vec_inconsistency],
                                                     inconsistency_reason = inconsistency_reason,
                                                     inconsistency_type = inconsistency_type,
                                                     rule_ID = rule_ID,
                                                     non_duplicated_error_type_per_record = rep(TRUE,length(vec_inconsistency)),
    ...
    # Retrieve the row indices and horizon_master from layers with known layer limits
    ...
    # Add a different number to each of the horizons in "layers" and "layers_vec"
    # (i.e. internally) so that they become unique
    ...
    # to store information about the redundant layers
      ...
      redundant_layers_summary <- data.frame(layer = layers_vec,
    ...
    # in "redundant_layers", and evaluate for each of them:
      ...
      for (j in vec_redundant) {
        ...
        if (any(redundant_layers$redundancy_type[vec_j] == 0)) {
          redundant_layers_summary$redundancy_type[j] <- 0} else
        ...
        if (any(redundant_layers$redundancy_type[vec_j] == 3)) {
          ...
          redundant_layers_summary$combined_layer[j] <- 
                                redundant_layers$combined_layer[vec_3]} else
          ...
          if (length(vec_2) == 2) {
            ...
            redundant_layers_summary$combined_layer[j] <- 
              paste0(redundant_layers$combined_layer[vec_2[1]],"_",
                   redundant_layers$combined_layer[vec_2[1]])} else {
                     
                     redundant_layers_summary$redundancy_type[j] <- 2;
                     redundant_layers_summary$combined_layer[j] <- 
                       redundant_layers$combined_layer[vec_2]}} else 
      ...
      if (!identical(which(redundant_layers$redundancy_type == 0), integer(0))) { 
        
        # Store these layers as "redundant_layers_to_remove"
        
        redundant_layers_to_remove <- redundant_layers$layer[
                                    which(redundant_layers$redundancy_type == 0)];
        
        # Store the row indices in df of these layers
        
        vec_redundant <- NULL
        
        for (j in 1:length(redundant_layers_to_remove)) {
          vec_redundant <- c(vec_redundant, 
                             vec_nonempty[which(layers == redundant_layers_to_remove[j])])}
        ...
        # rank(inferior_layer_limits_sub) == rank(superior_layer_limits_sub)
        # rank(inferior_layer_limits_sub) == rank(df$horizon_number[vec_unique])
        df$horizon_number_unique[vec_unique] <- rank(inferior_layer_limits_sub);
        
        
        # Store information about the redundant_layers in "list_redundant_layers"
        # (this is not reported in PIR)
        
        inconsistency_reason <- 
          "Caution: the given layer is redundant in the depth profile (i.e. a complete profile can be composed with the remaining layers).";
        inconsistency_reason_vec <- rep(NA, length(vec));
        inconsistency_reason_vec[which(!is.na(
          match(layers_vec, redundant_layers_to_remove)))] <- inconsistency_reason;
        
        rule_ID <- rep(NA,length(vec));
        rule_ID[which(!is.na(match(layers_vec, redundant_layers_to_remove)))] <- 
              "[not in PIR]"
        
        list_redundant_layers <- rbind(list_redundant_layers,
                                       data.frame(survey_form = as.factor(rep(survey_form,length(vec))), 
                                                  partner = df$partner[vec], 
                                                  partner_code = df$partner_code[vec], 
                                                  country = df$country[vec],
                                                  ...
                                                  redundancy_type = redundant_layers_summary$redundancy_type,
                                                  combined_redundant_horizon_master = redundant_layers_summary$combined_layer,
      ...
      if (all(rank(df$horizon_number[vec]) == rank(df$horizon_limit_low[vec]))) {
      df$horizon_number_unique[vec] <- df$horizon_number[vec]} else
    ...
    vec_horizon_number <- vec_nonempty[order(df$horizon_number_unique[vec_nonempty])];
    ...
    {if (df$horizon_limit_low[j] !=
         df$horizon_limit_up[vec_horizon_number[match(j, vec_horizon_number) + 1]])
    ...
    if ((!is.na(df$horizon_limit_low[j]) & is.na(df$horizon_limit_low_orig[j])) |
    ...
    if ((!is.na(df$horizon_limit_up[j_next]) & is.na(df$horizon_limit_up_orig[j_next])) |
    ...
    list_layer_inconsistencies <- rbind(list_layer_inconsistencies, 
                                        data.frame(survey_form = as.factor(rep(survey_form,length(j_next))), 
                                                   ...
                                                   code_line = df$code_line[j_next],
                                                   parameter = as.factor(rep("horizon_limit_up",length(j_next))), 
                                                   parameter_unit = as.factor(rep("cm",length(j_next))),
                                                   parameter_value = parameter_value_error,
                                                   inconsistency_reason = inconsistency_reason,
                                                   inconsistency_type = inconsistency_type,
    ...
    close(progress_bar)} # end of "pfh" part
}
